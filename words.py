words = {
    "Christmas": "рождество",
    "Easter": "Пасха",
    "Gypsy": "цыган, цыганский",
    "ID": "удостоверение личности",
    "Jew": "еврей",
    "Muslim": "мусульманин, мусульманский",
    "abandon": "оставить, забросить",
    "abate": "уменьшаться, ослабевать",
    "abduct": "похищать",
    "abide": "смириться, выносить",
    "able": "способный, могущий",
    "abnormal": "ненормальный, аномальный",
    "aboard": "на борту",
    "abolish": "упразднять",
    "abort": "прерывать",
    "abound": "изобиловать",
    "about": "примерно",
    "abroad": "за границу",
    "abrupt": "крутой, резкий",
    "absent": "отсутствующий",
    "absorb": "впитывать, поглощать",
    "abundant": "обильный",
    "abuse": "оскорблять, бранить, брань, злоупотреблять, насилие, домогательство",
    "accelerate": "ускорять",
    "accept": "принять",
    "access": "доступ",
    "accessory": "принадлежность, аксессуар",
    "accident": "случайность, происшествие, напр.  ДТП",
    "acclaim": "приветствовать, аплодировать",
    "accommodate": "размещать, вмещать",
    "accompany": "сопровождать",
    "accomplish": "выполнить, завершить",
    "accord": "согласие",
    "according to": "в соответствии",
    "account": "счёт (в банке), отчёт",
    "account for": "объяснить (произошедшее)",
    "accurate": "точный",
    "accuse": "обвинять",
    "accustom": "приучить",
    "ache": "боль, болеть",
    "achieve": "достигать",
    "acid": "кислота",
    "acknowledge": "признавать, подтверждать",
    "acquaint": "знакомить",
    "acquire": "обретать",
    "acquit": "оправдать (в суде)",
    "across": "за, через",
    "act": "действовать",
    "act on": "действовать в соответствии",
    "acute": "острый",
    "adamant": "непреклонный",
    "add to": "увеличивать",
    "add up": "сходиться (об информации)",
    "addict": "наркоман, привыкать",
    "addition": "добавление, сложение",
    "adhere": "придерживаться",
    "adjacent": "смежный, прилегающий",
    "adjective": "прилагательное",
    "adjust": "настраивать, поправлять и т.д.",
    "admire": "восхищаться",
    "admit": "признавать, допускать",
    "adolescent": "подросток, подростковый",
    "adopt": "принимать, перенимать, усыновить или взять (животное)",
    "adore": "обожать",
    "adorn": "украшать",
    "adult": "взрослый",
    "adultery": "прелюбодеяние, неверность",
    "advance": "продвигаться, продвижение",
    "advantage": "преимущество",
    "adventure": "приключение",
    "adverb": "наречие",
    "adversary": "противник, соперник",
    "adverse": "неблагоприятный",
    "advertise": "рекламировать",
    "advice": "совет (неисчисл!)",
    "advise": "советовать",
    "advocate": "защищать, отстаивать",
    "aerial": "воздушный, антенна",
    "affair": "дело, занятие, роман, любовная история",
    "affect": "воздействовать",
    "affection": "привязанность, влечение",
    "affiliate": "филиал, отделение, мл. партнёр",
    "affirm": "утверждать, подтверждать",
    "afflict": "поражать, вредить",
    "affluent": "обеспеченный, состоятельный",
    "afford": "позволить себе",
    "afraid": "напуганный",
    "after all": "всё-таки, в конце концов",
    "aftermath": "последствие, результат",
    "age": "век, эпоха, возраст",
    "agenda": "повестка дня",
    "aggravate": "обострять",
    "aggregate": "совокупный",
    "agile": "проворный, гибкий",
    "agitate": "волновать",
    "agree": "соглашаться",
    "agreement": "соглашение, договор",
    "agriculture": "сельское хозяйство",
    "ahead": "вперёд, впереди",
    "aid": "помощь, помогать",
    "ailment": "недомогание, болезнь",
    "aim": "цель",
    "air": "вид, впечатление",
    "airborne": "находящийся в воздухе",
    "aircraft": "самолёт, авиация",
    "airline": "авиалиния",
    "aisle": "проход",
    "akin": "похожий",
    "alarm": "тревога, сигнализация",
    "alas": "увы",
    "albeit": "хотя",
    "alert": "тревога, бдительный",
    "alien": "чужак, пришелец, иммигрант",
    "alienate": "отчуждать, отдалять, отвергать",
    "align": "выравнивать, выстраивать",
    "alike": "подобно",
    "alive": "живой",
    "all along": "всё время",
    "allege": "утверждать, приписывать",
    "allegiance": "верность",
    "allocate": "распределять (ресурсы)",
    "allot": "выделять, отводить",
    "allow": "разрешать",
    "allow for": "выделять, рассчитывать",
    "allowance": "выплата, пособие",
    "allude": "намекать, указывать",
    "allure": "очарование, очаровывать",
    "ally": "союзник",
    "almost": "почти",
    "alone": "в одиночестве",
    "alongside": "рядом",
    "aloud": "вслух",
    "alter": "изменять",
    "alternate": "чередовать, чередующийся",
    "although": "хотя и",
    "altitude": "высота",
    "altogether": "в целом",
    "amateur": "любитель, любительский",
    "amaze": "поражать, изумлять",
    "ambassador": "посол",
    "amber": "янтарь",
    "ambiguous": "двусмысленный, неоднозначный",
    "ambulance": "скорая помощь",
    "ambush": "засада",
    "amend": "исправить, улучшить",
    "amenity": "удобство, обустройство",
    "amid (amidst)": "среди",
    "ammunition": "боеприпасы",
    "amount": "количество (неисчисл.)",
    "ample": "обильный, широкий",
    "amplify": "усиливать",
    "amuse": "развлечь, порадовать",
    "ancestor": "предок",
    "anchor": "якорь, закреплять",
    "ancient": "древний",
    "anew": "заново",
    "anger": "гнев",
    "angle": "угол, угловой",
    "angry": "злой",
    "anguish": "мука, страдание",
    "animate": "живой, одушевлённый, оживлять",
    "ankle": "лодыжка, щиколотка",
    "anniversary": "годовщина",
    "announce": "объявлять",
    "annoy": "раздражать, надоедать",
    "annual": "ежегодный",
    "ant": "муравей",
    "anthem": "гимн",
    "anticipate": "ожидать",
    "antique": "старинный",
    "anxiety": "беспокойство, тревога",
    "anxious": "озабоченный, встревоженный",
    "anymore": "больше не",
    "anytime": "в любой момент",
    "anyway": "в любом случае",
    "anywhere": "где угодно, куда угодно",
    "apart": "отдельно, врозь",
    "apart from": "помимо",
    "apartment": "квартира",
    "ape": "обезьяна (крупная)",
    "apiece": "каждый, за штуку",
    "apology": "извинение(я)",
    "appal": "потрясать, ужасать",
    "apparent": "явный, видимый",
    "appeal": "обращаться, взывать",
    "appear": "появиться, казаться",
    "applause": "аплодисменты",
    "appliance": "прибор, приспособление",
    "apply": "применять, применяться, обращаться (с заявлением)",
    "appoint": "назначать, утверждать",
    "appraise": "оценивать",
    "appreciate": "ценить",
    "apprentice": "ученик, стажёр",
    "approach": "приближаться, подходить",
    "appropriate": "присвоить, подходящий, присущий",
    "approve": "утвердить, одобрить (офиц.)",
    "approve of": "одобрять (субъективно)",
    "approximate": "приблизительный",
    "apron": "фартук, передник",
    "apt": "склонный, способный",
    "aquatic": "водный",
    "arbitrary": "произвольный",
    "arc": "дуга",
    "arch": "арка, изгибать",
    "archive": "архив",
    "area": "область, участок",
    "argue": "спорить, доказывать",
    "arise (arose, arisen)": "возникать, происходить",
    "arm": "рука",
    "armor (armour)": "броня, доспехи",
    "arms": "оружие",
    "arouse": "вызывать, пробуждать",
    "arrange": "организовать, уладить и т.д.",
    "array": "ряд, порядок, строй",
    "arrest": "задерживать, тормозить",
    "arrive": "прибывать",
    "arrogant": "высокомерный",
    "arrow": "стрела, стрелка",
    "arson": "поджог",
    "article": "статья, вещь, изделие",
    "artificial": "искусственный",
    "as though": "как будто",
    "as well": "также",
    "as well as": "так же как и",
    "ascend": "восходить",
    "ascertain": "выяснить, установить",
    "ash": "пепел, зола",
    "ashamed": "пристыженный",
    "ashtray": "пепельница",
    "aside": "в стороне, в сторону",
    "aside from": "кроме",
    "ask around": "поспрашивать",
    "ask out": "пригласить сходить",
    "asleep": "спящий",
    "aspire": "стремиться",
    "ass": "жопа",
    "assassin": "убийца",
    "assault": "нападение, нападать",
    "assemble": "собирать",
    "assert": "утверждать, отстаивать",
    "assess": "оценить",
    "asset": "актив, имущество",
    "assign": "назначать, присваивать",
    "assimilate": "усваивать",
    "assist": "помогать",
    "associate": "общаться, связываться",
    "assume": "принимать на себя, предполагать",
    "assure": "уверять",
    "astonish": "изумлять",
    "astound": "изумлять",
    "asylum": "убежище",
    "at all": "совсем",
    "at first": "сначала",
    "at last": "наконец",
    "at least": "по крайней мере",
    "at odds with": "в противоречии, в конфликте",
    "at once": "сразу",
    "at the moment": "сейчас",
    "atop": "наверху",
    "atrocity": "зверство, злодеяние",
    "attach": "прикрепить, привязать",
    "attack": "нападать, нападение",
    "attain": "достигать",
    "attempt": "попытка, пытаться",
    "attend": "уделить внимание, посетить",
    "attic": "чердак",
    "attitude": "отношение",
    "attorney": "адвокат, прокурор, поверенный",
    "attract": "привлекать, притягивать",
    "attribute": "приписывать, относить",
    "audible": "слышимый",
    "audience": "аудитория (люди)",
    "augment": "увеличить, нарастить",
    "aunt": "тётя",
    "authorities": "органы власти",
    "authority": "власть",
    "authorize": "дать полномочия, поручение",
    "available": "доступный, в наличии",
    "avalanche": "лавина",
    "average": "средний",
    "avert": "предотвращать, отводить",
    "avid": "заядлый, рьяный, алчный",
    "avoid": "избегать",
    "await": "ожидать",
    "awake": "проснуться, бодрствующий",
    "award": "награда, премия, присуждать",
    "aware": "осведомлённый",
    "awe": "трепет, благоговение",
    "awesome": "здорово, классно",
    "awful": "ужасный",
    "awhile": "какое-то время",
    "awkward": "неуклюжий",
    "axe (ax)": "топор",
    "axis": "ось",
    "bachelor": "бакалавр, холостяк",
    "back down": "отступить, отказаться",
    "back then": "тогда",
    "back up": "поддержать, прикрыть",
    "backbone": "хребет",
    "backdrop": "фон",
    "background": "происхождение, предыстория и т.д., фон",
    "backpack": "рюкзак",
    "badge": "значок",
    "baffled": "в недоумении, в тупике",
    "bag": "сумка, мешок",
    "bail": "залог",
    "bait": "приманка",
    "bake": "печь (выпекать)",
    "bald": "лысый, плешивый",
    "bale": "тюк",
    "balloon": "воздушный шар",
    "ballot": "голосование, бюллетень",
    "ban": "запрещать",
    "band": "ансамбль",
    "bandage": "повязка",
    "bang": "удар, взрыв",
    "banish": "отгонять",
    "bank": "берег (реки, озера)",
    "bank on": "надеяться, полагаться",
    "bankrupt": "банкрот, неплатежеспособный",
    "banner": "знамя, транспарант",
    "baptize": "крестить",
    "bar": "пластина, брусок, прут и т. д., загораживать",
    "barb": "шип, зазубрина",
    "barber": "парикмахер",
    "bare": "голый",
    "barefoot": "босиком",
    "barely": "едва, лишь только",
    "bargain": "сделка, торговаться",
    "bark": "кора, лаять",
    "barn": "сарай",
    "barrel": "бочка",
    "barren": "бесплодный",
    "bartender": "бармен",
    "base": "основа, основывать",
    "basement": "подвал",
    "bash": "колотить",
    "basket": "корзина",
    "bastard": "ублюдок",
    "bat": "бита, летучая мышь",
    "batch": "партия, серия, группа",
    "bath": "ванна",
    "bathe": "купаться, купать",
    "batter": "паста, тесто, взбивать",
    "battle": "битва",
    "bay": "залив",
    "be after": "иметь целью",
    "be back": "возвращаться",
    "be in": "участвовать",
    "be into": "любить, увлекаться",
    "be keen to": "желать, стремиться",
    "be late": "опоздать",
    "be off": "отсутствовать",
    "be on fire": "гореть",
    "be over": "закончиться",
    "be up to": "замышлять что-то",
    "bead": "шарик, бусинка",
    "beak": "клюв",
    "beam": "балка, перекладина, луч",
    "bean": "боб",
    "bear (bore, born)": "рожать, нести, втч перен.",
    "beard": "борода",
    "beast": "зверь, животное, скот",
    "beat (beat, beaten)": "победить, бить",
    "beat up": "побить",
    "beauty": "красота или красавица",
    "beaver": "бобр",
    "beckon": "манить",
    "become of": "случиться с к-л впоследствии",
    "beef": "говядина",
    "beep": "гудок, гудеть, пищать",
    "beetle": "жук",
    "beforehand": "заранее",
    "beg": "просить",
    "behave": "вести себя",
    "behavior": "поведение",
    "behold": "созерцать, видеть",
    "beige": "бежевый",
    "belief": "убеждение, вера",
    "bell": "колокол, колокольчик",
    "bellow": "реветь, рёв",
    "belly": "живот",
    "belong to": "принадлежать",
    "beloved": "любимый",
    "below": "ниже, под",
    "belt": "пояс",
    "bench": "скамейка",
    "bend (bent, bent)": "изгибать, изгиб",
    "bend down": "наклониться",
    "beneath": "ниже, под",
    "benefit": "польза, создавать пользу",
    "benevolent": "благожелательный",
    "berry": "ягода",
    "berth": "койка, место, полка в поезде",
    "beside": "рядом",
    "besides": "кроме",
    "bestow": "даровать",
    "bet": "ставка, пари, держать пари",
    "betray": "предавать",
    "beverage": "напиток",
    "beware": "остерегаться",
    "bewilder": "смущать, сбивать с толку",
    "beyond": "вне, свыше",
    "bias": "склонность, предвзятость",
    "bicycle": "велосипед",
    "bid": "заявка, предложение цены",
    "bigot": "фанатик",
    "bill": "счёт (к оплате), банкнота",
    "billboard": "рекламный щит",
    "billion": "миллиард",
    "bin": "ящик, ведро, ларь",
    "bind (bound, bound)": "связывать",
    "binge": "кутёж, загул, пьянка",
    "birth": "рождение",
    "bishop": "епископ",
    "bitch": "сука",
    "bite (bit, bitten)": "кусать, укус",
    "bitter": "горький",
    "bizarre": "странный, причудливый",
    "black out": "обесточить",
    "blackmail": "шантажировать",
    "blade": "лезвие",
    "blame": "винить",
    "bland": "пресный, мягкий",
    "blank": "пустой, чистый",
    "blanket": "одеяло, покрывало",
    "blast": "дуновение, порыв, взрыв, взрывать",
    "blatant": "явный, вопиющий",
    "blaze": "полыхать, пламя",
    "bleach": "отбелить, обесцветить",
    "bleak": "мрачный",
    "bleed (bled, bled)": "кровоточить",
    "blend": "смешивать",
    "bless": "благословлять",
    "blind": "слепой",
    "blink": "мерцать, мигать",
    "bliss": "блаженство",
    "blizzard": "метель, буран",
    "bloat": "раздуваться",
    "block": "кусок, глыба и т. д., квартал",
    "block out": "блокировать, не пускать",
    "block up": "забиться (о канале)",
    "blonde": "блондинка",
    "bloom": "расцвет, расцветать",
    "blossom": "цвести, цвет",
    "blouse": "блузка",
    "blow (blew blown)": "дуть, удар",
    "blow out": "задуть (свечу)",
    "blow over": "пройти, утихнуть",
    "blow up": "взорвать, накачивать, надувать",
    "bluff": "блеф",
    "blunt": "тупой, притуплять",
    "blur": "размывать, смазывать",
    "blush": "румянец, краснеть",
    "board": "совет, коллегия, доска, борт, подниматься на борт",
    "boast": "хвастать, хвастовство",
    "boat": "лодка, судно",
    "bog": "болото, трясина",
    "boil": "кипеть, кипятить, варить",
    "bold": "смелый, дерзкий, безрассудный",
    "bolster": "укреплять, поддерживать",
    "bond": "связь, узы, соединение, обязательство, облигация",
    "bone": "кость",
    "bonfire": "костер",
    "boobs": "титьки",
    "book": "купить, забронировать",
    "boom": "заграждение, пояс",
    "boost": "повышать, усиливать",
    "boot": "сапог, ботинок",
    "booth": "будка, кабина",
    "booze": "выпивка",
    "border": "граница, край",
    "bore": "надоедать",
    "borough": "район, место",
    "borrow": "брать взаймы",
    "both": "оба",
    "bother": "беспокоить",
    "bottom": "дно",
    "boulder": "валун",
    "bounce": "отскакивать, прыгать",
    "bound": "обязан",
    "boundary": "граница, черта",
    "bounds": "границы, рамки (перен.)",
    "bounty": "вознаграждение",
    "bout": "всплеск, приступ, бой, поединок",
    "bow": "бант, бабочка, лук, поклон, наклоняться",
    "bow out": "откланяться, уйти",
    "bowel": "кишка",
    "bowl": "миска",
    "brace": "скоба, распорка, скрепа",
    "bracket": "скобка",
    "brag": "хвастаться, кичиться",
    "braid": "коса или оплётка",
    "brake": "тормоз",
    "branch": "ветвь, филиал, ответвление",
    "brass": "латунь",
    "brassiere (bra)": "бюстгальтер",
    "brave": "храбрый",
    "brawl": "шумная драка, скандал",
    "brazen": "наглый",
    "breach": "брешь, прорывать, пробивать",
    "breadth": "ширина, широта",
    "break": "перерыв, перемена",
    "break (broke, broken)": "нарушать",
    "break away": "отделяться",
    "break down": "ломаться",
    "break into (break in)": "проникнуть, вломиться",
    "break out": "вспыхнуть, разразиться, сбежать, вырваться",
    "break up": "разорвать отношения",
    "breakthrough": "прорыв, достижение",
    "breast": "грудь",
    "breath": "дыхание, вздох",
    "breathe": "дышать",
    "breed": "порода, поколение и т д.",
    "breed (bred, bred)": "разводить, выращивать",
    "brew": "заваривать, варить напиток",
    "bribe": "взятка",
    "brick": "кирпич",
    "bride": "невеста",
    "bridge": "мост",
    "brief": "краткий",
    "briefcase": "портфель",
    "bright": "яркий",
    "brilliant": "блестящий",
    "brim": "край сосуда",
    "bring (brought, brought)": "принести",
    "bring along": "взять с собой",
    "bring around": "убедить",
    "bring back": "вернуть",
    "bring down": "сбрасывать, свергать",
    "bring in": "привлечь",
    "bring out": "выпустить (новинку и т.д.)",
    "bring up": "поднять тему, воспитывать",
    "brink": "грань, край",
    "brisk": "энергичный, оживлённый",
    "bristle": "щетина",
    "brittle": "ломкий",
    "broad": "широкий",
    "broadcast": "транслировать, трансляция",
    "broom": "метла, веник",
    "broth": "бульон, отвар",
    "bruise": "синяк, ушиб",
    "brush": "кисть, щётка, чистить",
    "brute": "животное, зверь, грубый",
    "bubble": "пузырь",
    "buck": "самец, обычно рогатых",
    "bucket": "ведро, бадья",
    "buckle": "пряжка",
    "buckle up": "пристегнуться",
    "bud": "бутон или почка растения",
    "buddy": "дружище, парень",
    "budge": "сдвинуть",
    "buffalo": "буйвол или бизон",
    "bug": "жук",
    "build on": "нарастить, развить",
    "built-in": "встроенный",
    "bulb": "лампа, колба",
    "bulge": "бугор, выпуклость",
    "bulk": "основная масса",
    "bullet": "пуля",
    "bullshit": "ерунда, чушь",
    "bully": "задира, задирать",
    "bum": "бомж или бездельник",
    "bump": "шишка, выпуклость, удар, ударять",
    "bun": "булочка",
    "bunch": "связка, пучок, группа",
    "bundle": "связка, пучок, пачка",
    "bunk": "койка, спальное место",
    "bunny": "кролик (разг, детск.)",
    "burden": "бремя, нагружать",
    "burglar": "вор взломщик",
    "burn": "жечь",
    "burn down": "сгореть, сжечь (полностью)",
    "burn out": "выгореть, перегореть",
    "burrow": "нора, зарываться",
    "burst": "взрыв, всплеск и т. д.",
    "bury": "хоронить",
    "bush": "куст",
    "bustle": "суета, суматоха",
    "busy": "занятый",
    "butcher": "мясник",
    "butt": "торец, зад, задница",
    "butt in": "встревать, вмешиваться",
    "butter": "масло сливочное",
    "butterfly": "бабочка",
    "button": "пуговица, застёгивать",
    "buy": "верить, соглашаться (разг.)",
    "buy up": "скупить",
    "buzz": "жужжать",
    "by chance": "случайно",
    "by far": "безусловно",
    "by heart": "наизусть",
    "by oneself": "один, сам",
    "by the way": "кстати",
    "bypass": "обходить, обход",
    "cab": "такси",
    "cabbage": "капуста",
    "cabin": "хижина",
    "cable": "канат, кабель, трос и т.д.",
    "cage": "клетка",
    "calf (pl:calves)": "теленок",
    "call back": "перезвонить",
    "call for": "требовать, взывать",
    "call off": "отменить",
    "call on": "обратиться, попросить",
    "calm": "спокойный, успокоить",
    "calm down": "успокоиться",
    "camp": "лагерь",
    "can": "банка",
    "cancel": "отменять",
    "cancer": "рак (болезнь)",
    "candid": "честный, беспристрастный",
    "candle": "свеча",
    "candy": "конфеты",
    "cane": "трость",
    "cannon": "пушка",
    "canopy": "навес, тент, полог",
    "canvas": "холст",
    "cap": "кепка, шляпа, колпачок",
    "capable": "способный к ч-л.",
    "capacity": "ёмкость, вместимость, способность, возможность",
    "cape": "мыс",
    "capital": "главный, большой, высший",
    "captive": "пленник",
    "capture": "захватить",
    "carbohydrate": "углевод",
    "carbon": "углерод, уголь, графит",
    "carcass": "туша",
    "cardboard": "картон",
    "care": "забота, уход, переживать, интересоваться",
    "career": "карьера",
    "caress": "ласкать, гладить",
    "cargo": "груз, грузовой",
    "carpenter": "плотник",
    "carpet": "ковер",
    "carriage": "каретка",
    "carrot": "морковь",
    "carry": "носить",
    "carry out": "выполнить",
    "cart": "телега",
    "cartoon": "мультфильм",
    "carve": "вырезать (напр. из дерева)",
    "case": "случай, дело, ящик, футляр, чехол",
    "cash": "наличка, обналичить",
    "casket": "шкатулка или гроб (амер.)",
    "cast (cast, cast)": "бросать, метать, отливать",
    "casual": "случайный",
    "casualty": "жертва, потери",
    "catch": "расслышать, уловить",
    "catch (caught, caught)": "ловить",
    "catch on": "становиться популярным, внезапно понять",
    "catch up on": "следить быть в курсе",
    "catch up with": "догнать",
    "cater": "обслуживать, угождать",
    "catering": "общественное питание",
    "cathedral": "собор",
    "cattle": "крупный рогатый скот",
    "cause": "причина, быть причиной",
    "caution": "предосторожность, предостережение",
    "cave": "пещера, полость",
    "caviar": "икра",
    "cavity": "полость, впадина",
    "cease": "прекращать",
    "cedar": "кедр",
    "ceiling": "потолок, перекрытие",
    "celebrate": "праздновать",
    "celebrity": "знаменитость",
    "celestial": "астрономический, небесный",
    "cell": "ячейка, клетка",
    "cellar": "подвал",
    "cellular": "сотовый",
    "cemetery": "кладбище",
    "censorship": "цензура",
    "census": "перепись",
    "century": "век",
    "cereal": "злак, каша, хлопья",
    "certain": "определённый, уверенный",
    "chain": "цепь",
    "chair": "стул",
    "chairman": "председатель",
    "chalk": "мел",
    "challenge": "вызов, необычная задача",
    "chamber": "кабинет, зал, палата, камера",
    "chancellor": "канцлер",
    "change": "сдача, мелочь",
    "chant": "петь, скандировать, песнопение",
    "chaos": "хаос",
    "chap": "парень",
    "chapel": "часовня",
    "chapter": "глава",
    "character": "персонаж",
    "charge": "заряжать, нагружать, обвинять, плата",
    "charity": "благотворительность",
    "charm": "обаяние, очаровывать",
    "chart": "схема, график, таблица",
    "chase": "погоня, гнаться",
    "chat": "болтать",
    "chatter": "болтать, болтовня",
    "cheat": "жульничать",
    "cheat on": "изменять",
    "check": "проверить",
    "check in": "вселиться (в отель)",
    "check into": "лечь (в больницу)",
    "check on": "проверить, что всё норм",
    "check out": "посмотреть, заценить, освободить номер",
    "checkup": "медосмотр",
    "cheek": "щека",
    "cheer": "аплодировать, радостно встречать",
    "cheer on": "болеть (за спорт)",
    "cheer up": "взбодриться",
    "cheerful": "весёлый, неунывающий",
    "chef": "шеф-повар",
    "chemistry": "химия",
    "cherish": "лелеять, хранить",
    "chess": "шахматы",
    "chest": "грудь (грудная клетка), сундук",
    "chestnut": "каштан",
    "chew": "жевать",
    "chief": "вождь, начальник, главный",
    "childhood": "детство",
    "chill": "холод, охлаждать",
    "chill out": "расслабиться",
    "chimney": "дымовая труба",
    "chin": "подбородок",
    "chip": "щепка, стружка",
    "choice": "выбор",
    "choir": "хор",
    "choke": "задыхаться,  давиться",
    "chop": "рубить",
    "chop down": "срубить (дерево)",
    "chop up": "порезать (на кусочки)",
    "chord": "аккорд",
    "chore": "скучная неприятная работа",
    "chorus": "припев",
    "chuck": "бросать (небрежно)",
    "chuckle": "посмеиваться",
    "chunk": "кусок",
    "church": "церковь",
    "cinema": "кино, кинотеатр",
    "cinnamon": "корица",
    "circle": "круг, окружность",
    "circuit": "схема, цепь, контур",
    "circumstance": "обстоятельство",
    "circus": "цирк",
    "cite": "упоминать, цитировать",
    "citizen": "гражданин",
    "civic": "гражданский",
    "civil": "гражданский",
    "civilian": "гражданский",
    "claim": "требовать, претендовать, утверждать, претендовать",
    "clam": "моллюск",
    "clamour": "шум, ропот, протесты",
    "clamp": "зажимать, зажим",
    "clap": "хлопать в ладоши",
    "clarify": "прояснять",
    "clash": "столкновение",
    "clasp": "застёжка, застёгивать",
    "clause": "пункт, статья документа",
    "claw": "коготь, клешня",
    "clay": "глина",
    "clean up": "убрать (мусор)",
    "cleanse": "очищать",
    "clear": "ясный, чистый (пр. и перен.)",
    "clear off": "убраться вон",
    "clear out": "убирать, забирать",
    "clear up": "прояснить(ся)",
    "cleave": "раскалывать",
    "clench": "стиснуть, сжать",
    "clergy": "духовенство",
    "clerical": "канцелярский, административный",
    "clever": "умный",
    "click": "щёлкать, щелчок",
    "cliff": "утес",
    "climax": "кульминация, разгар",
    "climb": "взбираться",
    "cling (clung, clung)": "цепляться",
    "clip": "отсекать, обрезать, зажим, зажимать",
    "cloak": "плащ, мантия, покров",
    "clockwise": "по часовой стрелке",
    "clog": "загромождать",
    "close down": "закрыться (насовсем)",
    "close in on": "приближаться",
    "closet": "стенной шкаф",
    "closure": "закрытие, завершение",
    "cloth": "ткань, тряпка",
    "clothe": "одевать, покрывать",
    "cloud": "облако",
    "clout": "влияние, влиятельность",
    "clove": "гвоздика",
    "clue": "ключ, нить, улика",
    "clump": "комок, кучка, клок",
    "clumsy": "неловкий, неуклюжий",
    "clutch": "сцеплять, сцепление",
    "clutter": "беспорядок, куча всего",
    "co-worker": "сотрудник",
    "coach": "автобус, вагон, телега, тренер",
    "coal": "уголь",
    "coarse": "грубый, крупный, шероховатый",
    "coast": "побережье",
    "coaster": "подставка",
    "coat": "покрывать, пальто",
    "coax": "убедить, соблазнить сделать",
    "coconut": "кокос",
    "coerce": "принуждать",
    "coffin": "гроб",
    "coherent": "согласованный, связный",
    "cohesion": "единство, согласие",
    "coil": "катушка, моток, наматывать",
    "coincide": "совпасть",
    "coincidence": "совпадение",
    "cold": "простуда",
    "collaborate": "сотрудничать",
    "collapse": "обрушиться, сложиться",
    "collar": "воротник или ошейник",
    "collect": "собирать",
    "collide": "сталкиваться",
    "colon": "двоеточие",
    "colonel": "полковник",
    "column": "столб, колонна",
    "comb": "расческа, причёсывать",
    "combat": "бой",
    "combine": "соединять(ся)",
    "come about": "произойти",
    "come across": "встретить (случайно)",
    "come along": "идти вместе, продвигаться (о работе)",
    "come by": "найти, заполучить, зайти",
    "come down": "сводиться",
    "come from": "быть из",
    "come into": "унаследовать",
    "come off": "выйти, получиться, отвалиться",
    "come on": "включаться",
    "come out": "выступить, заявить, выходить (напр. об альбоме)",
    "come over": "заходить (в гости)",
    "come through": "пережить, перенести",
    "come to": "очнуться, доходить до",
    "come to life": "оживать, приходить в себя",
    "come true": "сбываться",
    "come under": "подвергнуться",
    "come up": "произойти или появиться, приближаться (о событии)",
    "come up to": "подойти к кому-л.",
    "come up with": "придумать",
    "come with": "прилагаться",
    "comedian": "комик",
    "comfort": "утешать, утешение",
    "commemorate": "отметить, почтить",
    "commence": "начинать",
    "commend": "высоко оценивать",
    "commercial": "реклама (на ТВ)",
    "commission": "поручение, полномочие",
    "commit": "совершить (напр. ошибку)",
    "commitment": "приверженность, обязательство",
    "commodity": "товар, сырьё",
    "common": "общий, обычный, распространённый",
    "commonplace": "обычное дело, банальность",
    "commonwealth": "содружество",
    "communicate": "общаться",
    "community": "община, сообщество",
    "commute": "ходить, курсировать",
    "compact": "договор",
    "compare": "сравнить",
    "comparison": "сравнение",
    "compartment": "отсек, отделение, купе",
    "compassion": "сочувствие, сострадание",
    "compatible": "совместимый",
    "compel": "вынуждать, заставлять",
    "compete": "соперничать",
    "complain": "жаловаться",
    "complement": "дополнять, дополнение, набор, состав, комплект",
    "complete": "завершить, выполнить",
    "complex": "сложный",
    "complicate": "усложнять",
    "comply": "соблюдать, соответствовать",
    "compose": "составлять, сочинять",
    "compound": "состав, составной",
    "comprehend": "понять, вникнуть",
    "comprehensive": "исчерпывающий, всесторонний",
    "compress": "сжимать, прессовать",
    "comprise": "состоять из, охватывать",
    "compulsory": "принудительный, обязательный",
    "comrade": "товарищ",
    "conceal": "скрывать",
    "concede": "уступить",
    "conceive": "помыслить, представить",
    "concept": "понятие, идея",
    "concern": "забота, тревожить",
    "conclude": "заключать, завершать",
    "concrete": "бетон, бетонировать",
    "concur": "соглашаться",
    "condemn": "осуждать",
    "condition": "состояние, условие",
    "conduct": "вести, поведение",
    "cone": "конус",
    "confer": "дать, принести, присудить, совещаться",
    "confess": "признаваться",
    "confide": "доверить, вверить",
    "confident": "уверенный",
    "confine": "ограничить (ся)",
    "confirm": "подтвердить",
    "conform": "соответствовать",
    "confront": "противостоять, сталкиваться",
    "confuse": "путать",
    "congestion": "загруженность, запор",
    "congratulate": "поздравлять",
    "conjunction": "соединение, связь",
    "connect": "соединять",
    "conquer": "завоевать",
    "conquest": "завоевание",
    "conscience": "совесть, сознательность",
    "conscious": "сознательный, сознающий",
    "consecutive": "последующий по времени",
    "consent": "согласие, разрешение",
    "consequence": "последствие",
    "consequent": "последующий, вытекающий",
    "conserve": "беречь, экономить",
    "consider": "рассматривать, учитывать, считать чем-либо",
    "considerable": "крупный, значительный",
    "consist of": "состоять (из)",
    "consistent": "последовательный",
    "console": "утешать",
    "conspicuous": "заметный",
    "conspire": "сговариваться",
    "constant": "постоянный",
    "constellation": "созвездие",
    "constitute": "составлять, образовывать",
    "constrain": "ограничивать (препятствием)",
    "construct": "соорудить",
    "construction (site)": "стройка",
    "consult": "советоваться",
    "consume": "потреблять",
    "contain": "содержать",
    "contaminate": "загрязнять",
    "contemplate": "созерцать или размышлять",
    "contemporary": "современный, современник",
    "contempt": "презрение, презирать",
    "contend": "бороться, соперничать",
    "content": "содержимое",
    "content with": "довольный, довольство",
    "contest": "соревнование, конкурс",
    "contingency": "непредвиденные обстоятельства",
    "continue": "продолжить",
    "contract": "сокращаться",
    "contradict": "противоречить",
    "contrary": "противоположный, вопреки",
    "contribute": "способствовать, вносить вклад",
    "controversy": "спор, разногласие, дискуссия",
    "convene": "созывать, собирать",
    "convenient": "удобный",
    "conventional": "стандартный, общепринятый",
    "converge": "сходиться, сближаться",
    "converse": "разговаривать",
    "convert": "преобразовывать",
    "convey": "перевозить, передавать",
    "convict": "осуждённый, осуждать",
    "conviction": "убеждение, убеждённость",
    "convince": "убеждать",
    "cookie": "печенье",
    "cool": "прохладный, свежий",
    "cool down": "остыть",
    "cooperate": "сотрудничать",
    "cope with": "совладать, справиться",
    "copper": "медь",
    "cord": "верёвка, шнур",
    "core": "ядро, сердцевина, сущность",
    "cork": "пробка",
    "corn": "кукуруза, зерно",
    "corner": "угол",
    "corps": "корпус",
    "corpse": "труп",
    "correct": "правильный, исправлять",
    "correlate": "соотносить",
    "correspond": "соответствовать",
    "corrupt": "развращать, разлагать",
    "cost (cost, cost)": "стоить, стоимость",
    "cotton": "хлопок",
    "couch": "диван",
    "cough": "кашель",
    "council": "совет (орган)",
    "counsel": "совет, консультация",
    "count": "считаться, иметь значение, считать",
    "count on": "рассчитывать на",
    "counter": "противостоять, противоположный, стойка, прилавок",
    "counterfeit": "подделка, фальшивый",
    "counterpart": "аналог, коллега, и т. д.",
    "countryside": "сельская местность",
    "county": "графство, округ",
    "coup": "переворот",
    "couple": "пара",
    "courage": "мужество",
    "court": "суд, двор (королевский), игровая площадка",
    "courtesy": "учтивость, этикет",
    "courtyard": "внутренний двор",
    "cover": "накрывать, покрывать",
    "coverage": "освещение",
    "covet": "желать, вожделеть",
    "coward": "трус",
    "cozy": "уютный",
    "crack": "трескаться, трещина",
    "crackle": "потрескивание",
    "cradle": "колыбель",
    "craft": "ремесло, мастерство",
    "cram": "втискивать, впихивать",
    "cramp": "судорога",
    "crane": "подъёмный кран",
    "crank": "чудак, фрик",
    "crap": "дерьмо",
    "crash": "грохот, крушение, разбивать и т. д.",
    "crate": "ящик",
    "crave": "желать, жаждать, требовать",
    "crawl": "ползать",
    "crayon": "цветной карандаш, мелок",
    "creak": "скрипеть",
    "cream": "крем, сливки",
    "crease": "складка, сминать",
    "create": "создавать",
    "creature": "существо",
    "credential": "удостоверение, аккредитация",
    "credible": "заслуживающий доверия",
    "creed": "вероисповедание, убеждение",
    "creek": "ручей",
    "creep": "гад, урод (разг)",
    "creep (crept, crept)": "ползти, красться",
    "creepy": "жуткий",
    "crest": "гребень",
    "crew": "команда, экипаж",
    "crib": "раскладушка или дет. кроватка",
    "crime": "преступление",
    "criminal": "преступник",
    "crimson": "малиновый, багряный",
    "cringe": "съёживаться",
    "cripple": "калека, калечить",
    "crisp": "хрустящий",
    "crook": "сгибать, искривлять",
    "crop": "урожай",
    "cross": "пересекать, пересечение, крест",
    "cross out": "вычеркнуть",
    "crossroads": "перекрёсток, перепутье",
    "crouch": "присесть, припасть к земле",
    "crow": "ворона",
    "crowd": "толпа",
    "crown": "корона, короновать",
    "crucial": "решающий, критический",
    "crude": "сырой, грубый, необработанный",
    "cruel": "жестокий",
    "crumb": "крошка, крупица",
    "crumble": "крошить, рушиться",
    "crumple": "смять",
    "crunch": "хрустеть",
    "crusade": "крестовый поход",
    "crush": "раздавливать, дробить",
    "crust": "кора, корка",
    "cry out": "закричать (от страха, боли)",
    "cub": "детеныш",
    "cubicle": "отсек, кабинка",
    "cucumber": "огурец",
    "cuddle": "обнимать, прижимать",
    "cue": "сигнал, намёк",
    "cuisine": "кухня",
    "cup": "чашка, кружка, кубок",
    "cupboard": "шкаф, буфет",
    "curb": "узда, обуздать, сдержать, бордюр или обочина",
    "cure": "лечить",
    "curious": "любопытный",
    "curl": "скручивать, клубиться, завиток",
    "currency": "валюта",
    "current": "текущий, актуальный, ток, поток, течение",
    "curriculum vitae": "резюме, биография",
    "curse": "проклятие",
    "curtain": "занавеска, занавес",
    "curve": "изгибать, кривая",
    "cushion": "подушка (маленькая)",
    "custody": "охрана, стража или опека",
    "custom": "заказной, индивидуальный, обычай, привычка",
    "customer": "клиент",
    "customs": "таможня",
    "cut across": "пересечь, срезать путь",
    "cut down": "урезать, уменьшить",
    "cut off": "отрезать, отключить",
    "cute": "привлекательный, милый",
    "dagger": "кинжал",
    "daily": "ежедневно",
    "dairy": "молочные продукты",
    "dam": "плотина",
    "damage": "вред, повреждать",
    "damn": "проклятый, проклинать",
    "damp": "сырой, сырость",
    "dangle": "мотаться",
    "dare": "осмелиться",
    "darling": "дорогой/ая (устар.)",
    "darn": "штопать",
    "dash": "тире, чёрточка, бросать, бросаться, разрушить (перен.)",
    "data": "данные",
    "database": "база данных",
    "date": "свидание, встречаться",
    "daunt": "пугать, обескураживать",
    "dawn": "рассвет",
    "dawn on": "осенить",
    "day off": "выходной",
    "daze": "ошеломить",
    "dazzle": "ослепить",
    "dead end": "тупик",
    "deaf": "глухой",
    "deal": "сделка",
    "deal in": "заниматься (напр. торговать)",
    "deal with": "иметь дело с",
    "debate": "обсуждать, спорить",
    "debris": "мусор, обломки, завалы",
    "debt": "долг",
    "decade": "десятилетие или десяток",
    "decay": "распад, упадок, разлагаться",
    "deceased": "покойный, усопший",
    "deceive": "обманывать",
    "decent": "приличный, достойный",
    "deception": "обман",
    "decide": "решить",
    "decisive": "решающий, решительный",
    "deck": "палуба, крышка, настил",
    "declare": "объявлять",
    "decline": "отказаться, отклонить, снижаться, снижение",
    "decorate": "украшать",
    "decrease": "уменьшать(ся), уменьшение",
    "decree": "постановление, декрет, указ",
    "dedicate": "посвящать",
    "deduct": "вычитать (об. про деньги)",
    "deed": "дело, поступок",
    "deem": "считать (чем-либо)",
    "deep": "глубокий",
    "deer (pl:deer)": "олень",
    "defeat": "поражение, наносить поражение",
    "defence (defense)": "защита, защитный",
    "defend": "защитить",
    "deficient": "недостаточный, неполный",
    "define": "определять",
    "definite": "определённый",
    "deflect": "отклонять, преломлять",
    "deft": "ловкий, проворный",
    "defy": "пренебрегать, отрицать",
    "degree": "степень (во всех значениях), градус",
    "deity": "божество",
    "delay": "задерживать(ся)",
    "deliberate": "обдуманный, преднамеренный",
    "delicate": "тонкий, нежный, хрупкий",
    "delicious": "восхитительный (о еде)",
    "delight": "восторг",
    "delinquent": "правонарушитель",
    "deliver": "доставлять, рожать",
    "delusion": "наваждение, заблуждение",
    "demand": "требовать, спрос",
    "demise": "кончина, конец",
    "demolish": "сносить",
    "den": "логово, берлога",
    "denote": "обозначать",
    "denounce": "осуждать, разоблачать",
    "dense": "густой",
    "dent": "вмятина",
    "dental": "зубной, стоматологический",
    "deny": "отрицать",
    "depart": "отправляться, отходить",
    "department": "отдел",
    "depend": "зависеть",
    "dependent": "зависимый",
    "depict": "изображать, описывать",
    "deplete": "истощать",
    "deploy": "развёртывать",
    "depot": "хранилище, база, склад",
    "depress": "угнетать, подавлять",
    "deprive": "лишать",
    "depth": "глубина",
    "deputy": "депутат, представитель",
    "derive": "выводить, извлекать",
    "descend": "спускаться",
    "descend from": "происходить от",
    "describe": "описать",
    "desert": "пустыня, бросать, оставлять",
    "deserve": "заслуживать",
    "designate": "обозначать, называть",
    "desire": "желание, желать",
    "desk": "письменный стол",
    "desktop": "настольный",
    "desolate": "пустынный, безлюдный",
    "despair": "отчаяние, безнадёжность",
    "desperate": "отчаянный, безвыходный",
    "despise": "презирать",
    "despite": "несмотря на",
    "destination": "место назначения",
    "destiny": "судьба, предназначение",
    "destroy": "разрушать",
    "detach": "отделить, оторвать",
    "detain": "задерживать",
    "detect": "обнаружить, выявить",
    "detention": "задержание, арест",
    "deter": "удерживать (от действия)",
    "deteriorate": "ухудшать",
    "determine": "определить",
    "determined": "решителен",
    "detriment": "ущерб, вред",
    "devastate": "опустошать, разорять",
    "develop": "развивать, разрабатывать",
    "deviate": "отклоняться",
    "devise": "придумывать, изобретать",
    "devoid": "лишенный",
    "devote": "посвятить, уделить",
    "devour": "пожирать",
    "dial": "набирать номер",
    "diamond": "бриллиант обычно алмаз",
    "diary": "дневник",
    "dice (pl: dice)": "кубик (игральный)",
    "dictionary": "словарь",
    "die down": "утихнуть",
    "die out": "вымереть",
    "differ": "отличаться",
    "different": "разный, отличающийся",
    "difficulty": "трудность",
    "diffuse": "рассеивать, распылять",
    "dig (dug, dug)": "копать",
    "dig up": "откопать",
    "digest": "переваривать",
    "digit": "цифра",
    "dignity": "достоинство",
    "diligent": "усердный, старательный",
    "dilute": "разбавлять, разводить",
    "dim": "тусклый",
    "dimension": "измерение, размер",
    "diminish": "уменьшаться",
    "dine": "ужинать",
    "dip": "окунать, погружать",
    "dire": "страшный, тяжёлый",
    "direct": "прямой (обычн. перен.знач.), направлять",
    "directory": "справочник",
    "disable": "отключить, нейтрализовать",
    "disabled": "инвалид (прил.)",
    "disappoint": "разочаровывать",
    "disaster": "бедствие",
    "discard": "отбросить, отказаться",
    "discern": "различить, распознать",
    "disciple": "ученик, последователь",
    "disclose": "раскрывать, обнаруживать",
    "discount": "скидка",
    "discourage": "отговорить, демотивировать",
    "discourse": "речь, рассуждение",
    "discover": "открыть (новое)",
    "discreet": "сдержанный",
    "discrepancy": "несоответствие, расхождение",
    "discrete": "отдельный, разрозненный",
    "discretion": "благоразумие, усмотрение",
    "discriminate": "различать, разделять",
    "discuss": "обсуждать",
    "disdain": "пренебрегать, презирать",
    "disease": "болезнь",
    "disgrace": "бесчестие, позор",
    "disguise": "маскировать, маскировка",
    "disgust": "отвращение",
    "dish": "тарелка или миска, блюдо",
    "disillusion": "разочаровывать",
    "dislocate": "вывихнуть, сместить",
    "dismantle": "разобрать, демонтировать",
    "dismay": "испуг, тревога, потрясение",
    "dismiss": "прогонять, посылать, увольнять, отклонить, отбросить",
    "disorder": "беспорядок",
    "disparity": "неравенство",
    "dispatch": "отправлять, отправка",
    "dispel": "развеяться, исчезнуть",
    "dispense": "распределять, раздавать",
    "dispense with": "обойтись без, избавиться",
    "disperse": "рассеиваться",
    "displace": "вытеснять, смещать",
    "display": "показ(ывать), демонстрировать",
    "dispose of": "располагать, распоряжаться, избавиться",
    "dispute": "спор, спорить",
    "disrupt": "нарушить, сорвать",
    "dissent": "несогласие, не соглашаться",
    "dissipate": "рассеивать",
    "dissolve": "растворять, растворяться",
    "distant": "отдалённый",
    "distinct": "чёткий, различимый",
    "distinguish": "выделять, отличать",
    "distort": "искажать",
    "distract": "отвлекать",
    "distress": "несчастье, страдание",
    "distribute": "распределять",
    "district": "район, округ и т. п.",
    "disturb": "тревожить",
    "ditch": "ров",
    "dive": "нырять",
    "diverge": "расходиться",
    "diverse": "разнообразный",
    "divert": "отвлечь, развлечь",
    "divide": "делить",
    "divine": "божественный",
    "divorce": "развод, разводиться",
    "dizzy": "головокружительный",
    "do about": "сделать (с чем-то)",
    "do away with": "покончить, избавиться",
    "do up": "привести в порядок",
    "do without": "обойтись без",
    "dock": "причал, док",
    "dodge": "уворачиваться, уклоняться, уловка",
    "dole": "пособие по безработице",
    "doll": "кукла",
    "domain": "область, сфера, владение",
    "dome": "купол, свод",
    "domestic": "домашний, внутренний (о политике)",
    "donate": "жертвовать, скидываться",
    "donkey": "осел",
    "doom": "гибель, рок, обречённость и т. д.",
    "doorway": "дверной проем",
    "dope": "наркотик, дурман",
    "dormitory": "общежитие",
    "dot": "точка",
    "double": "удваивать, двойной",
    "doubt": "сомневаться, сомнение",
    "dough": "тесто",
    "doughnut": "пончик",
    "downtown": "центр города",
    "downturn": "спад",
    "dozen": "дюжина",
    "draft": "тяга, сквозняк, проект, черновик, набросок",
    "drag": "тащить, волочить",
    "drag on": "тянуться долго",
    "drain": "слив, водосток",
    "drastic": "радикальный, резкий",
    "draw (drew, drawn)": "тянуть, влечь",
    "draw up": "составить (документ)",
    "drawback": "недостаток",
    "drawer": "выдвижной ящик",
    "dread": "страх, бояться",
    "dream": "сон (видение)",
    "dream up": "придумать (в деталях)",
    "dress": "одеваться",
    "dress up": "нарядиться",
    "drift": "дрейфовать или дрифтовать",
    "drill": "сверло, сверлить",
    "drills": "тренировка, учение",
    "drip": "капля, капать",
    "drive (drove, driven)": "водить машину, двигать, гнать",
    "drive out": "выгнать",
    "drizzle": "моросить, морось",
    "drop": "падать, ронять, капля",
    "drop by": "зайти (по пути)",
    "drop off": "завезти, высадить",
    "drop out of": "бросить (учёбу)",
    "drought": "засуха",
    "drown": "тонуть",
    "drug": "лекарство или наркотик",
    "drum": "барабан, барабанить",
    "dry": "сухой, сушить, засуха",
    "dry up": "пересохнуть, иссякнуть",
    "dual": "парный, двоякий",
    "dub": "дублировать",
    "dubious": "сомнительный",
    "duckling": "утёнок",
    "dude": "чувак",
    "due": "должный, положенный",
    "due to": "из-за, в связи",
    "duke": "герцог",
    "dull": "скучный, унылый",
    "dumb": "немой, тупой",
    "dummy": "манекен, чучело",
    "dump": "сваливать, сбрасывать свалка, бросать (парня/девушку)",
    "duo": "дуэт",
    "duplicate": "дублировать, дубликат",
    "durable": "прочный, долговечный",
    "duration": "продолжительность",
    "during": "во время",
    "dusk": "смеркаться",
    "dust": "пыль",
    "duty": "обязанность, служба",
    "dwarf": "карлик, гном",
    "dwell": "обитать, проживать",
    "dwell on": "остановиться (подробно)",
    "dwindle": "постепенно уменьшаться",
    "dye": "краска, краситель",
    "eager": "сильно желающий",
    "early": "рано, ранний",
    "earn": "зарабатывать",
    "earnest": "серьёзный, ревностный",
    "earring": "серьга",
    "earth": "Земля",
    "earthquake (quake)": "землетрясение",
    "ease": "лёгкость, облегчать",
    "east": "восток",
    "eat out": "есть не дома",
    "eat up": "съесть полностью",
    "echo": "эхо, отголосок, отражаться",
    "eclipse": "затмение",
    "edge": "край",
    "edible": "съедобный",
    "edit": "редактировать, монтировать",
    "educate": "воспитывать, обучать",
    "efficacy": "результативность",
    "efficient": "оптимальный, эффективный",
    "effort": "усилие, старание",
    "either": "либо, тоже (не), любой из двух",
    "eject": "выталкивать",
    "elaborate": "разработать, хорошо проработанный",
    "elbow": "локоть",
    "elder": "старший",
    "elect": "избирать",
    "elevate": "поднимать, повышать",
    "elicit": "добиться, извлечь",
    "eligible": "имеющий право",
    "eliminate": "устранять, ликвидировать",
    "elk": "лось",
    "eloquent": "красноречивый, выразительный",
    "elude": "ускользнуть, увернуться",
    "embark on": "начать (что-то серьёзное)",
    "embarrass": "смущать, стеснять",
    "embassy": "посольство",
    "embed": "встроить, вставить",
    "embody": "олицетворять",
    "embrace": "обхватить, обнять, обхват",
    "emerge": "появляться",
    "emergency": "чрезвычайная ситуация",
    "eminent": "видный, уважаемый",
    "emit": "испускать, издавать",
    "emperor": "император",
    "emphasis": "акцент, внимание, ударение",
    "empire": "империя",
    "employ": "использовать, нанимать на работу",
    "empower": "давать возможность, право",
    "empty": "пустой, опустошать",
    "enable": "давать возможность",
    "enact": "вводить (напр. закон)",
    "enchant": "околдовать, очаровать",
    "enclose": "окружать, заключать",
    "encompass": "охватывать, заключать",
    "encounter": "встретиться, столкнуться",
    "encourage": "поддерживать, поощрять",
    "end up": "заканчивать чем-либо",
    "endanger": "подвергать опасности",
    "endeavor (endeavour)": "стремиться, стараться, попытка",
    "endorse": "одобрять, поддерживать",
    "endow": "наделять, даровать",
    "endure": "выдерживать",
    "enforce": "внедрить, навязать",
    "engage in": "заниматься, вступать, вовлекать",
    "engaged": "помолвленный (-ая)",
    "engender": "зарождать, порождать",
    "engine": "двигатель или машина",
    "enhance": "повысить, усилить, расширить",
    "enlarge": "увеличить, расширить",
    "enlighten": "просветить",
    "enlist": "привлечь (к делу)",
    "enormous": "огромный",
    "enrich": "обогащать",
    "enrol (enroll)": "записать (в члены), набрать",
    "ensemble": "ансамбль",
    "ensue": "следовать, вытекать",
    "ensure": "обеспечить, гарантировать",
    "entail": "влечь за собой, подразумевать",
    "enter": "войти",
    "enterprise": "предприятие, предпринимательство",
    "entertain": "развлекать",
    "entice": "соблазнять",
    "entire": "весь, целый",
    "entitle": "давать название, давать право (по заслугам)",
    "entity": "сущность",
    "entrepreneur": "предприниматель",
    "envelope": "конверт",
    "environment": "среда, окружение",
    "envision": "воображать",
    "envy": "зависть",
    "equal": "равный, равняться",
    "equation": "уравнение",
    "equip": "оснащать, снаряжать",
    "equity": "равноправие, справедливость",
    "eradicate": "уничтожить, искоренить",
    "erase": "стирать",
    "erect": "воздвигнуть, соорудить",
    "erode": "размывать, выветривать",
    "errand": "поручение",
    "error": "ошибка",
    "erupt": "извергаться",
    "escape": "избежать, спастись",
    "essence": "сущность",
    "essential": "необходимый",
    "establish": "установить, учредить",
    "esteem": "уважать, почитать, почтение",
    "estimate": "примерно оценивать",
    "eternal": "вечный",
    "evade": "уклоняться",
    "evaluate": "дать оценку",
    "evaporate": "испаряться",
    "eve": "канун",
    "even": "ровный",
    "event": "событие",
    "eventual": "конечный, возможный",
    "ever": "когда-либо",
    "evidence": "доказательство (неисчисл.)",
    "evident": "очевидный",
    "evil": "злой, зло",
    "evoke": "вызывать",
    "evolve": "развиваться, эволюционировать",
    "exact": "точный",
    "exaggerate": "преувеличивать",
    "examine": "осмотреть, изучить",
    "example": "пример",
    "excavate": "рыть, выкапывать",
    "exceed": "превышать",
    "excel": "преуспеть",
    "excellent": "превосходный",
    "except": "кроме, исключая",
    "excerpt": "выдержка, отрывок",
    "excess": "избыток, чрезмерный",
    "exchange": "обмен",
    "excite": "волновать, возбуждать",
    "exclaim": "восклицать",
    "exclude": "исключать",
    "excuse": "оправдание, отмазка",
    "execute": "исполнить, казнить",
    "exempt": "освободить (от), освобождение",
    "exercise": "упражнение, тренироваться",
    "exert": "приложить, оказать",
    "exhale": "выдыхать",
    "exhaust": "исчерпывать, истощать, выхлопные газы",
    "exhibit": "выставлять, показывать",
    "exile": "ссылка, изгнание",
    "exist": "существовать",
    "exit": "выход, выходить",
    "expand": "расширять, распространять",
    "expanse": "пространство, простор",
    "expect": "ожидать (события)",
    "expel": "высылать, изгонять",
    "expenditure": "расходы",
    "expense": "расходы",
    "experience": "опыт",
    "expertise": "опыт, компетенция",
    "expire": "истекать",
    "explain": "объяснять",
    "explicit": "явный, откровенный",
    "explode": "взрываться",
    "exploit": "использовать, эксплуатировать",
    "explore": "изучать",
    "expose": "выставлять, обнажать",
    "express": "выражать",
    "exquisite": "утончённый, изысканный",
    "extend": "расширять, удлинять",
    "extent": "степень, мера",
    "exterior": "внешний, внешняя сторона",
    "external": "внешний",
    "extinct": "вымерший",
    "extinguish": "погасить",
    "extort": "вымогать",
    "extra": "дополнительный",
    "extract": "извлекать",
    "extreme": "крайний",
    "exuberant": "буйный, энергичный",
    "eyebrow": "бровь",
    "eyelid": "веко",
    "fable": "басня",
    "fabric": "ткань, материал",
    "fabulous": "невероятный, легендарный",
    "face": "столкнуться (перен)",
    "face up to": "принять (обстоятельства)",
    "facet": "грань, в т. ч. перен. (аспект)",
    "facial": "лицевой",
    "facilitate": "облегчать, способствовать",
    "facility": "сооружение, объект, лёгкость, удобство",
    "faction": "группировка, фракция",
    "factory": "фабрика, завод",
    "faculty": "дар, способность",
    "fade": "увядать, блекнуть",
    "fail": "потерпеть неудачу",
    "failure": "неудача, провал",
    "faint": "слабый, неуловимый, обморок, падать в обморок",
    "fair": "светлый, справедливый, честный",
    "fairly": "довольно, вполне",
    "fairy": "фея",
    "faith": "вера",
    "fall": "осень (амер.)",
    "fall apart": "разваливаться",
    "fall asleep": "уснуть",
    "fall back on": "опереться, прибегнуть",
    "fall behind": "отставать",
    "fall for": "вестись, влюбляться",
    "fall in love with": "влюбиться",
    "fall out": "ссориться",
    "fall over": "упасть, перевернуться",
    "fall through": "провалиться",
    "false": "ложный, неверный",
    "falter": "колебаться, дрогнуть",
    "fame": "известность, слава",
    "familiar": "знакомый",
    "famine": "голод",
    "famous": "знаменитый",
    "fan": "вентилятор",
    "fancy": "необычный, причудливый, модный, представлять, полагать",
    "faraway": "далёкий",
    "fare": "плата за проезд",
    "farewell": "прощальный, прощание",
    "farm": "ферма",
    "fascinate": "очаровывать",
    "fashion": "мода, стиль, образ, манер",
    "fasten": "закрепить",
    "fat": "жир, жирный",
    "fate": "судьба, участь",
    "fatigue": "усталость, утомление",
    "faucet": "кран",
    "fault": "недостаток, неисправность, вина, ошибка",
    "favor": "благоприятствовать",
    "favor (favour)": "услуга",
    "fear": "страх, бояться",
    "feasible": "выполнимый, осуществимый",
    "feast": "праздник, пир",
    "feat": "подвиг, свершение",
    "feather": "перо",
    "feature": "черта, особенность",
    "fee": "плата",
    "feeble": "слабый, немощный",
    "feed (fed, fed)": "кормить",
    "feed on": "питаться",
    "feedback": "обратная связь",
    "feel (felt, felt)": "трогать, щупать",
    "feel for": "сочувствовать",
    "feel like": "хотеть",
    "fellow": "парень, человек, товарищ по чему-либо",
    "felon": "преступник, уголовник",
    "female": "женский",
    "feminine": "женский",
    "fence": "забор, огораживать",
    "fend for (oneself)": "позаботиться о (себе)",
    "fend off": "защищаться",
    "ferocious": "свирепый",
    "ferry": "переправа, паром",
    "fertile": "плодородный",
    "fetch": "сходить принести",
    "fetus": "плод, эмбрион",
    "fever": "лихорадка, жар",
    "fibre": "волокно",
    "fiction": "вымысел, худ.литература",
    "fiddle": "скрипка",
    "fidelity": "верность",
    "field": "поле, область",
    "fierce": "свирепый",
    "fight back": "отбиваться",
    "fight off": "отбить",
    "figure": "полагать, думать",
    "figure out": "понять, придумать",
    "fill in": "заполнить",
    "fill in on": "посвятить (в подробности)",
    "fill up": "наполнить целиком",
    "film": "снимать кино",
    "filth": "грязь, гадость",
    "fin": "плавник",
    "find out": "выяснить",
    "fine": "штраф, тонкий, хороший",
    "fingertip": "кончик пальца",
    "fir": "ель",
    "firefighter": "пожарник",
    "fireplace": "камин",
    "firework": "фейерверк, петарда",
    "firm": "твёрдый",
    "fisherman": "рыбак",
    "fishing": "рыбалка",
    "fist": "кулак",
    "fit": "приступ, припадок",
    "fit (fit, fit)": "подходить, помещаться",
    "fit in with": "вписаться (в коллектив)",
    "fix": "исправить, наладить",
    "fixture": "крепление, стойка и т.д.",
    "flair": "чутьё, талант",
    "flake": "слоиться, крошиться",
    "flakes": "хлопья",
    "flame": "пламя, пылать",
    "flank": "бок, фланг",
    "flap": "створка, клапан, хлопать",
    "flare": "вспышка, вспыхивать",
    "flash": "вспышка, вспыхнуть",
    "flashlight": "фонарик",
    "flat": "плоский, ровный, квартира",
    "flatter": "льстить",
    "flavor": "вкус, приправлять",
    "flaw": "изъян",
    "flea": "блоха",
    "flee (fled, fled)": "спасаться бегством",
    "fleet": "флот",
    "fleeting": "мимолётный, скоротечный",
    "flesh": "плоть, мясо",
    "flex": "сгибать",
    "flick": "короткое резкое движение, фильм (разг.)",
    "flick through": "просмотреть, пролистать",
    "flicker": "мерцать",
    "flight": "полёт, рейс, бегство",
    "flight&nbsp;attendant": "бортпроводник",
    "flinch": "дёрнуться, дрогнуть",
    "fling (flung, flung)": "швырнуть",
    "flip": "переворачивать, перевёрнутый",
    "float": "плыть",
    "flock": "стадо, стая",
    "flood": "потоп, наводнение, наводнять",
    "flood in": "наплыть, валить потоком",
    "floor": "пол, этаж",
    "flop": "провал, неудача",
    "flour": "мука",
    "flourish": "процветать",
    "flow": "поток, течь",
    "flu": "грипп",
    "fluctuate": "колебаться",
    "fluent": "беглый",
    "fluffy": "пушистый",
    "fluid": "жидкий, текучий",
    "flurry": "поток, шквал",
    "flush": "промывать, смывать, румянец, порозоветь",
    "flute": "флейта",
    "flutter": "трепетать, развеваться, порхать",
    "fly": "муха",
    "foam": "пена",
    "focus": "сосредоточить (ся)",
    "foe": "недруг",
    "fog": "туман",
    "foil": "расстаривать, разрушать, фольга",
    "fold": "сворачивать, складывать",
    "folder": "папка",
    "foliage": "листва",
    "folk": "люди, народ",
    "follow": "следовать, следить",
    "folly": "глупость, безумие",
    "fond of": "любящий",
    "fool": "дурак, дурачить",
    "fool around": "придуриваться",
    "footage": "запись, кадры видео",
    "footing": "опора, основа",
    "footprint": "след",
    "for real": "правда, серьёзно",
    "for sure": "наверняка, точно",
    "for the sake of": "ради",
    "forbid (forbade, forbidden)": "запрещать",
    "forcible": "насильственный",
    "forecast": "прогноз",
    "forehead": "лоб",
    "foreign": "иностранный",
    "foreman": "мастер, бригадир",
    "foremost": "передовой, первоочередной",
    "foresee": "предвидеть",
    "forge": "ковать, подделывать",
    "forgive (forgave, forgiven)": "простить",
    "fork": "развилка, раздваиваться",
    "former": "бывший, первый (из названных)",
    "formidable": "внушительный, грозный",
    "forth": "дальше, вперёд",
    "forthcoming": "предстоящий",
    "fortify": "укреплять, поддерживать",
    "fortunate": "удачный",
    "fortune": "состояние (деньги), счастье, удача, судьба",
    "fossil": "ископаемое",
    "foster": "приёмный (ребёнок), способствовать, питать и т.д.",
    "foul": "гадкий, скверный, грязный",
    "found": "основать, учредить",
    "foundation": "фундамент",
    "fountain": "фонтан",
    "foyer": "фойе",
    "fraction": "доля, часть, дробь",
    "fracture": "перелом, разрушение",
    "fragile": "хрупкий",
    "fragrant": "душистый, ароматный",
    "frail": "хилый",
    "frame": "рамка, каркас и т.п.",
    "framework": "каркас, основа, рамки",
    "frank": "откровенный",
    "frantic": "неистовый, бешеный",
    "fraud": "обман, мошенничество",
    "fray": "драка, бой, соревнование",
    "frayed": "потрёпанный",
    "freak": "урод",
    "freak out": "пугать или бесить",
    "free": "свободный или бесплатный",
    "free up": "высвободить",
    "freeway": "автострада",
    "freeze (froze, frozen)": "замораживать",
    "freight": "грузовой",
    "frenzy": "неистовство, безумие",
    "frequent": "частый",
    "freshen up": "освежить(ся)",
    "fret": "волновать (ся)",
    "friction": "трение",
    "friendly": "дружелюбный",
    "fright": "испуг",
    "frighten off": "отпугивать",
    "fringe": "край, крайний",
    "frog": "лягушка",
    "front": "перед, передний",
    "frontier": "граница",
    "frost": "мороз",
    "frown": "хмуриться",
    "fruitless": "бесплодный",
    "frustrate": "расстраивать (план, схему)",
    "fry": "жарить",
    "fuck with": "дразнить, шутить, злить",
    "fuel": "топливо, заправлять",
    "fugitive": "беглец",
    "fulfil": "исполнить",
    "fumble": "щупать, лапать, мять и т. д.",
    "fume": "дым, испарение",
    "fund": "фонд, финансировать",
    "funeral": "похороны, похоронный",
    "fungus": "гриб, грибок",
    "funnel": "воронка",
    "fur": "мех",
    "furnace": "печь",
    "furnish": "предоставлять, обставлять, меблировать",
    "furniture": "мебель",
    "further": "дальнейший, далее",
    "fury": "ярость, бешенство",
    "fuse": "плавиться, сливаться",
    "fuss": "суетиться",
    "futile": "тщетный, напрасный",
    "future": "будущий, будущее",
    "fuzzy": "пушистый, ворсистый",
    "gain": "получать, выигрывать",
    "gait": "походка",
    "gala": "праздник, торжество",
    "gamble": "азартная игра, играть",
    "gang": "банда, шайка",
    "gap": "разрыв, пробел",
    "garbage": "мусор (особ. пищевой)",
    "garden": "сад",
    "garlic": "чеснок",
    "garment": "предмет одежды",
    "garnish": "гарнир, украшение",
    "gas": "бензин",
    "gasp": "задыхаться, ловить воздух",
    "gates": "ворота",
    "gather": "собирать (ся)",
    "gauge": "измерить, изм. прибор",
    "gaze": "пристально глядеть",
    "gear": "механизм, привод, передача",
    "geared to": "направлен на",
    "gem": "(полу)драгоценный камень",
    "gender": "пол",
    "gene": "ген",
    "general": "общий",
    "generation": "поколение",
    "generous": "великодушный, щедрый",
    "gentle": "мягкий, тихий, нежный и т. д.",
    "genuine": "подлинный, настоящий",
    "germ": "микроб или зародыш",
    "gesture": "жест, жестикулировать",
    "get": "становиться",
    "get about": "передвигаться (на чём-то)",
    "get ahead": "преуспевать, продвигаться",
    "get ahead of": "обогнать",
    "get along": "ладить",
    "get at": "клонить, намекать",
    "get away with": "безнаказанно сделать",
    "get back at": "отомстить",
    "get by": "обойтись, протянуть",
    "get down": "приступить",
    "get in": "прибывать (о транспорте)",
    "get lost": "пропасть",
    "get off": "выходить (из транспорта)",
    "get off with": "отделаться чем-либо",
    "get on": "быть, делать",
    "get out of": "избежать, отмазаться",
    "get over": "пережить, принять",
    "get rid of": "избавиться от",
    "get stuck": "застрять",
    "get through": "дозвониться",
    "get together": "собираться",
    "get used to": "привыкнуть",
    "ghost": "привидение",
    "giant": "гигантский",
    "gift": "подарок",
    "giggle": "хихикать, хихиканье",
    "give a ride": "подвезти",
    "give away": "раздавать, дарить, выдать (раскрыть)",
    "give birth to": "родить",
    "give off": "издавать, испускать",
    "give out": "раздать, выдать",
    "give up": "отказаться, сдаться, бросить (привычку)",
    "glacier": "ледник",
    "glamour": "очарование, привлекательность",
    "glance": "взгляд",
    "glare": "яркий свет, ослеплять",
    "glass": "стекло или стакан",
    "glasses": "очки",
    "glaze": "глазурь, глянец",
    "gleam": "блеск, отблеск",
    "glide": "скользить, планировать",
    "glimpse": "вид (мельком или уголком)",
    "glint": "вспыхнуть, сверкнуть (слегка)",
    "glitter": "блеск, блестеть, сверкать",
    "globe": "мир, земной шар",
    "gloom": "сумрак, уныние",
    "glory": "слава, сияние",
    "gloss": "глянец, лоск",
    "glove": "перчатка",
    "glow": "сияние, свечение",
    "glue": "клей, клеить",
    "gnaw on": "грызть, обгладывать",
    "go about": "заниматься (делами)",
    "go against": "противоречить",
    "go ahead": "взять (и сделать)",
    "go along": "поддерживать, соглашаться",
    "go bad": "испортиться",
    "go down": "быть принятым",
    "go for": "быть проданным за, выбирать, предпочитать",
    "go into": "вдаваться",
    "go off": "сработать, отключиться",
    "go on": "происходить, продолжаться",
    "go out": "погаснуть",
    "go over": "просмотреть, изучить",
    "go through": "просмотреть, перебрать",
    "go together": "сочетаться",
    "go under": "разориться",
    "goal": "цель",
    "goat": "козел, коза",
    "goddess": "богиня",
    "goods": "товары",
    "goof": "балбес, придурок (амер.)",
    "gorgeous": "шикарный",
    "gospel": "евангелие",
    "gossip": "слухи",
    "govern": "управлять",
    "gown": "платье",
    "grab": "схватить, нахапать и т.д.",
    "grace": "благодать, милость",
    "grade": "оценка, класс, уровень",
    "gradual": "постепенный",
    "graduate": "выпускник, выпускаться",
    "grain": "зерно, крупица",
    "grand": "большой, великий",
    "grant": "давать, дарить",
    "grape": "виноград (ягода)",
    "graph": "график",
    "grapple": "сцепиться, бороться",
    "grasp": "схватывать",
    "grate": "решетка",
    "grateful": "благодарный",
    "gratify": "удовлетворять, радовать",
    "gratitude": "благодарность",
    "grave": "могила, серьёзный",
    "gravel": "гравий",
    "gravy": "подливка, соус",
    "graze": "пасти(сь)",
    "grease": "смазка, жир",
    "greed": "жадность",
    "greenhouse": "теплица, оранжерея",
    "greet": "приветствовать",
    "grid": "сетка, решётка",
    "grief": "горе",
    "grievance": "обида",
    "grieve": "горевать",
    "grim": "мрачный, зловещий",
    "grin": "ухмыляться, ухмылка, оскал",
    "grind (ground, groud)": "молоть, измельчать",
    "grip": "хватать, захват, хватка",
    "grit": "крошка, песок, гравий, твёрдость, стойкость",
    "groan": "стон, стонать",
    "grocery": "продуктовый",
    "groom": "жених",
    "groove": "канавка, желоб",
    "grope": "щупать, искать вслепую",
    "gross": "валовой, грязный, грубый (во всех смыслах)",
    "grove": "лесок",
    "grow (grew, grown)": "становиться, расти или выращивать",
    "grow up": "вырастать",
    "growl": "рычать",
    "grudge": "обида, злоба",
    "grumble": "ворчать, жаловаться",
    "grunt": "хрюкать",
    "guard": "сторожить, стража",
    "guess": "угадывать, догадываться",
    "guest": "гость",
    "guide": "вести, направлять",
    "guideline": "общее правило, принцип",
    "guilt": "вина",
    "guise": "облик, личина",
    "gulf": "разрыв, пропасть, залив",
    "gulp": "большой глоток, заглатывать",
    "gum": "жвачка",
    "gun": "огнестрельное оружие",
    "gust": "порыв",
    "gut": "кишка",
    "gutter": "желоб, сток",
    "gym": "спортзал",
    "habit": "привычка, обычай",
    "habitat": "среда, место обитания",
    "hack": "рубить, кромсать",
    "hail": "град, окликнуть, привлечь внимание, приветствовать",
    "haircut": "стрижка, причёска",
    "hall": "зал, вестибюль",
    "hallmark": "характерная черта",
    "halt": "останавливаться",
    "halve": "делить пополам",
    "ham": "ветчина",
    "hammer": "молоток",
    "hamper": "затруднять, ограничивать",
    "hand down": "передать (по наследству)",
    "hand in": "сдать",
    "hand over": "передать",
    "handbook": "руководство, справочник",
    "handcuffs": "наручники",
    "handful": "горсть",
    "handicap": "помеха, недостаток",
    "handkerchief": "носовой платок",
    "handle": "рукоятка, управляться, обращаться",
    "handsome": "красивый (обычно о мужчинах)",
    "handwriting": "почерк",
    "handy": "удобный",
    "hang (hung, hung)": "висеть, вешать",
    "hang around": "зависать, слоняться",
    "hang out": "тусоваться где-то",
    "hangover": "похмелье",
    "happen": "случиться, происходить",
    "harass": "тревожить, изводить",
    "harbor (harbour)": "гавань, порт",
    "hardly": "едва",
    "hardware": "аппаратные средства",
    "hardy": "выносливый",
    "harm": "вред, вредить",
    "harness": "запрягать, обуздывать",
    "harp": "арфа",
    "harsh": "резкий, суровый, грубый",
    "harvest": "урожай, собирать урожай",
    "hassle": "трудность, неудобство",
    "haste": "спешка",
    "hat": "шляпа",
    "hatch": "люк",
    "hatred": "ненависть",
    "haul": "тащить, тянуть, буксировать",
    "haunt": "являться, преследовать (перен.)",
    "hawk": "ястреб",
    "hay": "сено",
    "hazard": "опасность, риск, угроза",
    "haze": "дымка, туман",
    "head for": "направляться",
    "head off": "выйти, отправиться",
    "headlight": "передняя фара",
    "headline": "заголовок",
    "headquarters": "штаб-квартира",
    "heal": "лечить, заживать",
    "health": "здоровье",
    "healthcare": "здравоохранение",
    "heap": "куча",
    "heat": "тепло, нагревать",
    "heat up": "разогреть",
    "heave": "вздыматься",
    "heaven": "небеса",
    "hebrew": "еврейский",
    "hectic": "беспокойный, напряжённый",
    "hedge": "живая изгородь",
    "heed": "прислушиваться, учитывать",
    "heel": "каблук, пятка",
    "hefty": "большой и тяжёлый",
    "height": "высота, рост",
    "heir": "наследник",
    "helicopter": "вертолёт",
    "helm": "руль, штурвал",
    "helmet (helm)": "шлем",
    "help out": "выручить",
    "helpful": "полезный",
    "helpless": "беспомощный",
    "hemisphere": "полушарие",
    "hen": "курица",
    "hence": "отсюда (следует, происходит)",
    "herald": "возвестить, вестник",
    "herb": "трава",
    "herd": "стадо, пасти",
    "heritage": "наследие",
    "hero": "герой",
    "hesitate": "колебаться, медлить",
    "hide (hid, hidden)": "скрывать",
    "hideous": "отвратительный, ужасный",
    "hierarchy": "иерархия",
    "high-end": "высококлассный, лидирующий",
    "highlight": "выделить, подсветить",
    "highway": "шоссе",
    "hijack": "угонять",
    "hike": "поход, ходить в поход",
    "hilarious": "веселый, забавный, смешной",
    "hill": "холм",
    "hinder": "мешать, затруднять",
    "hinge": "петля, шарнир",
    "hint": "намёк, намекать",
    "hip": "таз, поясница",
    "hire": "нанимать",
    "hiss": "шипеть",
    "hit (hit, hit)": "ударить, удар, попасть, достичь",
    "hit back": "отыграться, отбиться",
    "hit on": "клеить, подкатывать",
    "hitch": "заминка, помеха",
    "hoard": "копить, накопление, клад",
    "hobble": "прихрамывать",
    "hog": "большой свин, боров",
    "hoist": "поднимать",
    "hold back": "сдерживать",
    "hold on": "оставаться на линии, держаться",
    "hold up": "задержать, затормозить",
    "hole": "дыра",
    "holler": "крикнуть, окликнуть",
    "hollow": "полый",
    "holy": "святой, священный",
    "homeland": "родина",
    "homicide": "убийство",
    "honest": "честный",
    "honey": "мёд",
    "honor (honour)": "честь, чтить",
    "hood": "капот, кожух, капюшон",
    "hoof": "копыто",
    "hook": "крюк, зацеплять",
    "hook up": "потрахаться, подключить, смонтировать",
    "hoop": "обруч, обод",
    "hop": "прыгать, скакать",
    "hopeful": "внушающий надежды",
    "horizon": "горизонт, кругозор",
    "horn": "рог",
    "horrible": "ужасный, отвратительный",
    "horrify": "ужасать",
    "horseback": "верхом",
    "hose": "шланг",
    "hospitality": "гостеприимство",
    "host": "хозяин, быть хозяином, множество",
    "hostage": "заложник",
    "hostile": "враждебный, неприязненный",
    "hound": "охотничья собака, об. гончая",
    "household": "домашний, домохозяйство",
    "housewife": "домохозяйка",
    "hover": "зависнуть, парить",
    "however": "однако",
    "howl": "выть",
    "huddle": "столпиться, скучиться",
    "hue": "цвет, оттенок",
    "hug": "объятие, обнимать",
    "huge": "огромный",
    "hull": "корпус, оболочка",
    "hum": "гудеть, жужжать",
    "human": "человек, человеческий",
    "humble": "скромный, смиренный",
    "humid": "влажный",
    "humiliate": "унижать",
    "hump": "горб",
    "hunch": "горб, горбиться, догадка, предчуствие",
    "hunger": "голод",
    "hunk": "ломоть, лакомый кусок",
    "hurdle": "препятствие",
    "hurl": "метать, бросать с силой",
    "hurricane": "ураган",
    "hurry": "спешить, торопиться",
    "hurt (hurt, hurt)": "причинять боль или болеть",
    "hush": "молчать",
    "hustle": "толкаться, торопиться, нечестно добыть (амер.)",
    "hut": "хижина, лачуга, барак",
    "hydrogen": "водород",
    "hypocrite": "лицемер",
    "idle": "праздный, бездействующий",
    "ignite": "воспламеняться",
    "ignorant": "невежественный",
    "ill": "больной, дурной, нехороший",
    "illicit": "незаконный, запрещённый",
    "illuminate": "освещать",
    "image": "изображение, образ",
    "imagine": "вообразить",
    "immediate": "немедленный, непосредственный",
    "immense": "огромный",
    "immerse": "погружаться (в т. ч. перен)",
    "imminent": "неизбежный",
    "impact": "воздействие, удар",
    "impair": "ухудшать, ослаблять и т.д.",
    "impede": "затруднять",
    "impending": "предстоящий, грозящий",
    "imperative": "обязательный, необходимый",
    "implement": "осуществлять",
    "implicate": "указать на причастность",
    "implicit": "подразумеваемый, неявный",
    "imply": "подразумевать",
    "importance": "важность",
    "impose": "навязывать, налагать",
    "imposing": "внушительный, импозантный",
    "impress": "впечатлить",
    "imprint": "отпечаток, отпечатывать",
    "improve": "улучшить",
    "in a row": "подряд",
    "in advance": "заранее",
    "in charge": "во главе",
    "in spite of": "несмотря на",
    "incentive": "стимул, мотив",
    "inch": "дюйм",
    "incidence": "частотность, распространённость",
    "incline": "наклонять, склонять (втч. перен)",
    "include": "включать, содержать в себе",
    "income": "доход",
    "incoming": "входящий",
    "incorporate": "вобрать, соединить",
    "increase": "увеличивать(ся), увеличение",
    "incredible": "невероятный",
    "incumbent": "возложенный",
    "incur": "навлечь, повлечь",
    "indeed": "в самом деле",
    "indicate": "указывать, означать",
    "indifferent": "безразличный",
    "indigenous": "коренной, местный",
    "induce": "вызывать, побуждать",
    "indulge": "баловать, потворствовать",
    "industry": "отрасль",
    "inevitable": "неизбежный",
    "inexplicable": "необъяснимый",
    "infamous": "печально известный, постыдный",
    "infant": "ребёнок",
    "infantry": "пехота",
    "infect": "заражать",
    "infer": "делать вывод",
    "inferior": "низший",
    "infinite": "бесконечный",
    "inflame": "разжигать (в осн. перен.)",
    "inflate": "надувать",
    "inflict": "причинять, наносить",
    "influence": "влиять, влияние",
    "infuse": "вливать, настаивать",
    "ingenious": "изобретательный",
    "inhabit": "населять, обитать в",
    "inhale": "вдыхать",
    "inherent": "свойственный, присущий",
    "inherit": "унаследовать, перенять",
    "inhibit": "препятствовать, затруднять",
    "initial": "первоначальный",
    "inject": "впрыскивать, вкачивать",
    "injure": "ранить, наносить ущерб",
    "ink": "чернила",
    "inland": "внутренний, вглубь страны",
    "inmate": "обитатель",
    "inn": "гостиница",
    "inner": "внутренний",
    "innocent": "невинный, невиновный",
    "input": "вход, ввод",
    "inquire": "узнавать, интересоваться",
    "insect": "насекомое",
    "insert": "вставить",
    "inside": "внутри",
    "inside out": "наизнанку",
    "insight": "понимание, догадка",
    "insist": "настаивать",
    "inspect": "проверять, осматривать",
    "inspire": "вдохновить, внушить",
    "install": "устанавливать",
    "instance": "пример, случай",
    "instant": "мгновение, мгновенный",
    "instead": "вместо",
    "insulate": "изолировать, оградить",
    "insult": "оскорбление, оскорблять",
    "insure": "страховать, гарантировать",
    "insurgent": "повстанец, мятежник",
    "intact": "неповрежденный",
    "intake": "потребление, приём",
    "integral": "составной, неотъемлемый",
    "integrity": "честность, порядочность, целостность",
    "intelligence": "разведка",
    "intelligent": "умный",
    "intelligible": "вразумительный, понятный",
    "intend": "намереваться",
    "intended": "предназначенный",
    "interact": "взаимодействовать",
    "intercept": "перехватить, перехватить",
    "intercourse": "отношения или пол. сношение",
    "interfere": "вмешиваться, мешать",
    "interim": "промежуточный, временный",
    "interior": "внутренность, внутренний",
    "intermediate": "промежуточный",
    "intern": "стажёр",
    "internal": "внутренний",
    "interpret": "истолковывать",
    "interpreter": "переводчик",
    "interrogate": "допрашивать",
    "interrupt": "прерывать, перебивать",
    "intersect": "пересекаться",
    "intervene": "вмешиваться",
    "intestine": "кишечник",
    "intimate": "близкий",
    "intimidate": "запугивать",
    "intricate": "сложный, запутанный",
    "intrinsic": "присущий, внутренний",
    "introduce": "представить, познакомить, ввести (в употребление)",
    "intrude on": "вторгаться, вмешиваться",
    "invade": "вторгаться",
    "invent": "изобретать",
    "inventory": "опись, учёт",
    "invert": "переворачивать, опрокидывать",
    "investigate": "расследовать",
    "invite": "приглашать",
    "invoke": "взывать, призывать",
    "involve": "вовлекать, увлекать, содержать в себе (процесс)",
    "inward": "внутрь",
    "iron": "утюг, гладить, железо, железный",
    "irritate": "раздражать",
    "island": "остров",
    "issue": "издать, выпустить, выпуск, вопрос, проблема",
    "itch": "зуд",
    "item": "пункт, номер, предмет",
    "ivory": "слоновая кость",
    "ivy": "плющ",
    "jab": "ударять, пихать",
    "jacket": "куртка, пиджак",
    "jail": "тюрьма",
    "janitor": "дворник, уборщик",
    "jar": "банка, кувшин и т. д.",
    "jaw": "челюсть",
    "jealous": "ревнивый",
    "jelly": "желе",
    "jeopardy": "опасность (серьёзная)",
    "jerk": "дёргать, резкое движение, придурок",
    "jet": "струя, реактивный двигатель/самолёт",
    "jewel": "драгоценности",
    "jingle": "звенеть, звон",
    "jog": "бегать трусцой",
    "join": "присоединить (ся)",
    "joint": "сустав, шарнир, стык и т. д., совместный, объединённый",
    "jolly": "веселый",
    "jolt": "толчок, встряска",
    "journalism": "журналистика",
    "journey": "путешествие",
    "joy": "радость",
    "judge": "судья, судить (втч. перен)",
    "judicial": "судебная",
    "jug": "кувшин",
    "jumble": "перемешивать",
    "jump at": "ухватиться (за возможность)",
    "jump in": "вмешаться",
    "jumper": "джемпер",
    "junction": "стык, узел, пересечение",
    "junior": "младший",
    "junk": "барахло, хлам",
    "junkie": "наркоман",
    "jury": "жюри или присяжные",
    "just": "справедливый, беспристрастный",
    "justice": "правосудие, справедливость",
    "justify": "оправдывать, обосновывать",
    "jut": "выступать, выдаваться",
    "keen": "острый (часто перен.)",
    "keep away": "держать (ся) подальше",
    "keep off": "избегать, не трогать",
    "keep on": "продолжать делать",
    "keep out of": "не влезать",
    "keep up": "успевать за чем-л.",
    "kernel": "ядро",
    "kettle": "чайник",
    "kick": "пинать",
    "kick around": "обсудить (разг.)",
    "kick back": "расслабиться",
    "kid": "шутить, разыгрывать, ребёнок",
    "kidnap": "похищать",
    "kidney": "почка",
    "kin (pl:kin)": "родственник",
    "kind": "добрый, тип, вид",
    "kindergarten": "детский сад",
    "kit": "комплект, набор",
    "kite": "коршун или воздушный змей",
    "kitten": "котенок",
    "knack": "умение, сноровка",
    "knee": "колено",
    "kneel": "стать на колени (колено)",
    "knight": "рыцарь",
    "knit": "вязать",
    "knob": "ручка (круглая)",
    "knock": "стучать",
    "knock down": "снести, сбить",
    "knock out": "вырубить (ударом)",
    "knock together": "слепить, соорудить",
    "knot": "узел",
    "knowledge": "знания",
    "label": "метка, ярлык, метить",
    "labour": "труд, трудиться",
    "lace": "шнурок, шнуровать",
    "lack": "недостаток, нуждаться",
    "lad": "парень",
    "ladder": "лестница",
    "laden": "груженный",
    "lag": "запаздывание",
    "lamb": "ягненок",
    "lame": "хромой",
    "lament": "оплакивать, сожалеть",
    "land": "приземляться, высаживаться",
    "landlord": "арендодатель, домовладелец",
    "landmark": "ориентир",
    "landscape": "пейзаж",
    "lane": "полоса дороги",
    "lantern": "фонарь",
    "lap": "колени",
    "lapse": "упущение, оплошность, промежуток времени",
    "laptop": "ноутбук",
    "large": "большой, широкий",
    "lash (eyelash)": "ресница",
    "lash out": "нападать, ругаться",
    "last": "длиться",
    "latch": "защёлка, засов",
    "late": "поздний, покойный",
    "lateral": "боковой",
    "latter": "последний (из названных)",
    "laugh": "смеяться",
    "laughter": "смех",
    "launch": "начать, запустить, запуск",
    "laundry": "прачечная",
    "lavish": "щедрый, роскошный",
    "law": "закон",
    "lawn": "газон",
    "lawsuit": "тяжба, судебное дело",
    "lay (laid, laid)": "класть, укладывать",
    "lay down": "установить (правило, норму)",
    "lay off": "уволить, сократить",
    "lay out": "выложить, разложить",
    "layer": "слой",
    "layout": "макет, расположение",
    "lazy": "ленивый",
    "lead": "вести",
    "lead up to": "предшествовать",
    "leaf (pl: leaves)": "лист",
    "leak": "утечка, просачиваться, протекать",
    "lean": "худой",
    "lean against": "опереться, прислонить",
    "lean over": "наклониться над, через",
    "leap": "прыгать, прыжок",
    "lease": "сдавать в аренду",
    "leash": "привязь, поводок",
    "leather": "кожаный, кожа",
    "leave (left, left)": "уйти, покинуть",
    "leave behind": "оставить, забыть",
    "ledge": "уступ",
    "leek": "лук-порей",
    "leftover": "остаток",
    "leg": "нога",
    "legacy": "наследие, наследство",
    "legal": "законный, правовой",
    "legislation": "законодательство",
    "legitimate": "законный, обоснованный",
    "leisure": "досуг, отдых",
    "lend (lent, lent)": "давать в долг",
    "length": "длина",
    "lens": "линза, объектив",
    "lessen": "уменьшать(ся)",
    "lest": "чтобы не, иначе",
    "let in on": "посвящать (в секрет, план)",
    "let off": "отпустить, дать отделаться",
    "let smb down": "подвести",
    "let smb in": "впустить",
    "let up": "прекратиться, стихнуть",
    "letter": "письмо",
    "lettuce": "салат (латук)",
    "level": "уровень",
    "lever": "рычаг, ручка",
    "liable": "подлежащий",
    "liaison": "связь",
    "liberate": "освобождать",
    "liberty": "свобода",
    "library": "библиотека",
    "licence (license)": "разрешение",
    "lick": "лизать",
    "lid": "крышка",
    "lie": "лгать, ложь",
    "lie (lay, lain)": "лежать",
    "lifelong": "пожизненный",
    "lift": "поднимать",
    "light (lit, lit)": "зажигать",
    "light up": "осветить или загореться",
    "lightning": "молния",
    "like": "как",
    "like that": "так",
    "likely": "вероятно",
    "likewise": "также",
    "limb": "конечность",
    "limit": "ограничивать, ограничение",
    "limp": "хромать",
    "line": "строка, линия, ряд",
    "line up": "встать в очередь",
    "lineage": "родословная",
    "linen": "бельё, льняная ткань",
    "lineup": "расстановка, расположение",
    "linger": "задерживаться, медлить",
    "link": "связь, звено, ссылка",
    "lip": "губа",
    "lipstick": "губная помада",
    "liquid": "жидкость, жидкий",
    "list": "список, перечислять",
    "listen in on": "подслушивать",
    "literal": "буквальный",
    "literary": "литературный, книжный",
    "literate": "грамотность",
    "litter": "мусор (на улице)",
    "live up to": "соответствовать, дотягивать",
    "livelihood": "средства к существованию",
    "liven up": "оживить, взбодрить",
    "liver": "печень",
    "livestock": "скот",
    "lizard": "ящерица",
    "load": "грузить, груз",
    "loaf": "буханка, булка",
    "loan": "заём, ссуда",
    "loathe": "ненавидеть, питать отвращение",
    "lobby": "вестибюль, фойе, прихожая",
    "lobster": "омар",
    "local": "местный",
    "locate": "определить местоположение",
    "lock": "замок, запирать",
    "lock up": "посадить в тюрьму или дурку",
    "locker": "ящик, шкаф (закрывающийся)",
    "locus": "местоположение",
    "lodge": "домик, поселиться, остановиться, подавать (документ)",
    "loft": "чердак, верхний нежилой этаж",
    "log": "бревно",
    "lone": "одинокий",
    "long ago": "давно",
    "long for": "желать или скучать",
    "longevity": "долговечность, долголетие",
    "look": "выглядеть",
    "look after": "присматривать",
    "look around": "посмотреть, оглядеть",
    "look at": "рассмотреть, обдумать",
    "look for": "искать",
    "look forward": "ожидать, предвкушать",
    "look into": "посмотреть, изучить",
    "look out": "смотреть (опасность!)",
    "look smb up": "найти (в списке и т. п.)",
    "look up": "улучшаться, налаживаться",
    "look up to": "уважать, восхищаться",
    "loom": "маячить, виднеться",
    "loop": "петля",
    "loose": "свободный, болтающийся",
    "loot": "грабить, добыча",
    "lord": "господин, господь (бог)",
    "loss": "потеря",
    "lot": "жребий, судьба",
    "loud": "громкий",
    "lounge": "гостиная, холл, комната отдыха, расслабляться, бездельничать",
    "lousy": "паршивый",
    "lovely": "чудесный, милый",
    "lower": "нижний",
    "loyal": "верный, преданный",
    "lucrative": "прибыльный, выгодный",
    "ludicrous": "нелепый, смехотворный",
    "luggage": "багаж",
    "lumber": "пиломатериалы",
    "lump": "кусок, глыба, ком",
    "lunar": "лунный",
    "luncheon": "завтрак",
    "lung": "легкое",
    "lurch": "крениться, шататься",
    "lure": "приманка, манить",
    "lurk": "таиться",
    "lush": "пышный",
    "lust": "вожделение, страсть",
    "luxury": "роскошь",
    "lyrics": "текст песни",
    "magazine": "журнал",
    "magician": "волшебник, фокусник",
    "magnificent": "великолепный, величественный",
    "magnify": "увеличивать",
    "magnitude": "величина, сила",
    "maid": "служанка, горничная",
    "maiden": "дева, девственница",
    "mainland": "материк или большой остров",
    "maintain": "поддерживать, сохранять",
    "majesty": "величие, величество",
    "major": "главный, большой",
    "make fun of": "смеяться, издеваться",
    "make into": "переделать в",
    "make of": "извлечь, получить",
    "make out": "разобрать, понять, целоваться",
    "make sure": "убедиться",
    "make up": "выдумать, помириться",
    "make up for": "компенсировать, наверстать",
    "makeup": "макияж",
    "male": "мужской",
    "mall": "торговый центр",
    "mammal": "млекопитающее",
    "mammoth": "мамонт",
    "manage": "справляться, управлять",
    "mandatory": "обязательный",
    "manifest": "проявить, явный",
    "mankind": "человечество",
    "mansion": "особняк, здание",
    "mantle": "мантия",
    "manual": "руководство, учебник, ручной",
    "manufacture": "производить, производство",
    "manure": "навоз",
    "map out": "запланировать, расписать",
    "maple": "клён",
    "marble": "мрамор",
    "margin": "край, граница, маржа, накрутка",
    "marine": "морской",
    "marital": "супружеский",
    "maritime": "морской",
    "mark": "метка, отмечать",
    "mark down": "снижать ценник",
    "marry": "жениться, выйти замуж",
    "marsh": "болото",
    "martial": "боевой, военный",
    "martyr": "мученик",
    "marvel": "диво, удивляться",
    "mash": "разминать, месиво",
    "massacre": "резня",
    "master": "хозяин, овладеть (напр. искусством)",
    "masterpiece": "шедевр",
    "mat": "коврик, циновка",
    "match": "соответствие, нечто соответствующее, спичка",
    "mate": "товарищ, компаньон",
    "maternal": "материнский",
    "matter": "вещество, материя, дело, вопрос, иметь значение",
    "mature": "зрелый, зреть",
    "may": "пусть",
    "mayor": "мэр",
    "maze": "лабиринт",
    "meadow": "луг",
    "meager": "скудный, недостаточный",
    "meal": "еда",
    "mean": "неважный, посредственный",
    "mean (meant, meant)": "иметь в виду, означать, предназначать",
    "means": "средство",
    "meantime": "тем временем",
    "meanwhile": "тем временем",
    "measure": "мера, мероприятие, измерять, измерение",
    "mediate": "посредничать",
    "medicine": "лекарство",
    "medieval": "средневековый",
    "mediocre": "посредственный",
    "meditate": "обдумывать или созерцать",
    "medium": "средство, способ",
    "meet (met, met)": "встретить или познакомиться",
    "melon": "дыня",
    "melt": "таять, плавить(ся)",
    "member": "член",
    "memoirs": "мемуары",
    "memories": "воспоминания",
    "memory": "память",
    "menace": "угроза, угрожать",
    "mend": "чинить, поправлять",
    "mental": "умственный",
    "mention": "упоминать",
    "mentor": "наставник",
    "merchandise": "товар",
    "merchant": "торговец, купец",
    "mercury": "ртуть",
    "mercy": "милость, милосердие",
    "mere": "всего лишь",
    "merge": "соединять, сливать",
    "merit": "заслуга, заслуживать",
    "merry": "веселый",
    "mesh": "сетка",
    "mess": "беспорядок, путаница",
    "mess up": "испортить, накосячить",
    "mess with": "возиться, связываться и т.д.",
    "metropolitan": "столичный",
    "mid": "середина, полу-",
    "middle": "середина, средний",
    "midnight": "полночь",
    "midst": "середина, среда",
    "midwife": "акушерка",
    "might": "мощь",
    "mild": "мягкий",
    "mileage": "пробег",
    "military": "военный",
    "mill": "мельница или фабрика",
    "millennium": "тысячелетие",
    "mimic": "передразнивать, копировать",
    "mince": "фарш, крошить, рубить",
    "mind": "возражать",
    "mindful": "внимательный, помнящий",
    "mine": "мина, шахта, рудник",
    "mingle": "смешиваться",
    "minor": "второстепенный, незначительный, несовершеннолетний",
    "mint": "мята",
    "miracle": "чудо",
    "mischief": "вред, безобразие",
    "miserable": "несчастный, жалкий",
    "misery": "нищета, несчастье, страдание",
    "miss": "скучать, пропускать, упустить",
    "missile": "ракета",
    "missing": "пропавший, отсутствующий",
    "mist": "туман",
    "mistake": "ошибка",
    "mistake for": "принимать за",
    "mistress": "госпожа, хозяйка, любовница",
    "mitigate": "смягчить, сгладить",
    "mix up": "перепутать",
    "mixture": "смесь",
    "moan": "стонать",
    "mob": "толпа (неуправляемая)",
    "mock": "издеваться, насмехаться",
    "mode": "режим, способ, вид",
    "moderate": "умеренный, средний",
    "modest": "скромный, умеренный",
    "modify": "видоизменять",
    "moist": "влажный",
    "mold (mould)": "плесень, форма, формировать",
    "mole": "крот, родинка",
    "molest": "приставать, досаждать",
    "monastery": "монастырь",
    "monetary": "валютный, денежный",
    "monk": "монах",
    "mood": "настроение",
    "moose": "лось",
    "mop": "швабра, мыть пол",
    "moreover": "более того",
    "moron": "дебил",
    "mortal": "смертельный, смертный",
    "mortgage": "ипотека",
    "mosque": "мечеть",
    "mosquito": "комар",
    "moss": "мох",
    "mostly": "в основном",
    "moth": "мотылёк или моль",
    "motion": "движение",
    "motto": "девиз",
    "mound": "насыпь, бугор",
    "mount": "монтировать, устанавливать, подниматься, забираться, гора",
    "mountain": "гора, горный",
    "mourn": "скорбеть, носить траур",
    "move": "переезжать",
    "move out": "съехать, выселиться",
    "move over": "подвинуться",
    "movie": "кино",
    "mow": "косить",
    "muck": "грязь, мусор, навоз",
    "mud": "грязь",
    "mug": "кружка",
    "multiple": "множественный, неединичный",
    "multiply": "умножать или размножаться",
    "multitude": "множество",
    "mumble": "бормотать, бубнить",
    "mural": "стенная роспись, фреска",
    "murder": "убийство, убивать",
    "murky": "мутный, тёмный",
    "murmur": "шум, ропот, журчание",
    "muscle": "мышца",
    "mushroom": "гриб",
    "mustache (moustache)": "усы",
    "mustard": "горчица",
    "muster": "собрать, согнать, мобилизовать",
    "mute": "беззвучный, не говорящий",
    "mutter": "бормотать, бурчать",
    "mutual": "взаимный",
    "muzzle": "морда или намордник",
    "mystery": "тайна",
    "nail": "гвоздь, прибивать, ноготь",
    "naked": "голый",
    "name after": "назвать в честь",
    "nanny": "няня",
    "nap": "вздремнуть, дремота",
    "napkin": "салфетка",
    "narrate": "повествовать, рассказывать",
    "narrow": "узкий",
    "nasty": "противный",
    "native": "родной, местный, туземный",
    "natural": "естественный",
    "naughty": "непослушный",
    "nausea": "тошнота",
    "navigate": "ориентироваться, ходить по курсу",
    "navy": "военно-морской флот",
    "neat": "аккуратный, опрятный",
    "necessary": "необходимый",
    "neck": "шея",
    "necklace": "ожерелье",
    "needle": "игла",
    "neglect": "пренебрегать, не следить",
    "negligent": "небрежный, халатный",
    "negotiate": "вести переговоры",
    "neighbour": "сосед",
    "neither": "ни один из двух, тоже не",
    "neither...nor": "ни",
    "nephew": "племянник",
    "nerd": "ботан, задрот",
    "nerve": "смелость",
    "nest": "гнездо, гнездиться",
    "net": "чистый, конечный, без вычетов",
    "network": "сеть",
    "never": "так и не",
    "nevertheless": "тем не менее",
    "newcomer": "новичок",
    "newspaper": "газета",
    "nibble on": "покусывать, обгрызать",
    "niche": "ниша",
    "niece": "племянница",
    "nightmare": "кошмар",
    "nipple": "сосок",
    "nitrogen": "азот",
    "no matter": "неважно",
    "noble": "благородный",
    "nod": "кивать, склонять",
    "noise": "шум",
    "nominate": "выдвигать, номинировать",
    "none": "никакой, ни один",
    "nonetheless": "тем не менее",
    "noodles": "лапша",
    "noon": "полдень",
    "north": "север, северный",
    "nose around": "рыскать, вынюхивать",
    "nostril": "ноздря",
    "notch": "выемка",
    "note": "заметка, запись",
    "notice": "заметить",
    "notify": "известить",
    "notion": "понятие, представление",
    "notorious": "известный, пресловутый",
    "notwithstanding": "несмотря на",
    "noun": "существительное",
    "nourish": "питать, кормить, удобрять",
    "novel": "роман",
    "novelty": "новинка",
    "nuclear": "ядерный",
    "nude": "обнаженный",
    "nudge": "подталкивать",
    "nuisance": "неприятность, неудобство",
    "numb": "онемелый, окоченевший",
    "numeric (numerical)": "числовой, цифровой",
    "numerous": "многочисленные",
    "nun": "монахиня",
    "nurse": "медсестра, нянька",
    "nurture": "растить, питать",
    "nut": "орех",
    "nutrient": "питательный, пит. вещество",
    "nutrition": "питание",
    "nuts": "чокнутый",
    "oak": "дуб",
    "oath": "клятва, присяга",
    "oats": "овес, овсянка",
    "obedience": "покорность",
    "obese": "страдающий ожирением",
    "obey": "подчиняться, слушаться",
    "object": "возражать",
    "objective": "цель",
    "oblige": "обязывать",
    "oblique": "косой, наклонный",
    "obscene": "непристойный",
    "obscure": "неясный, тёмный",
    "observe": "наблюдать",
    "obsess": "овладеть (разумом)",
    "obsolete": "устарелый",
    "obstacle": "препятствие",
    "obstruct": "препятствовать, загромождать",
    "obtain": "получить, обрести",
    "obvious": "очевидный",
    "occasion": "случай, событие",
    "occupation": "профессия, род деятельности",
    "occupy": "занимать",
    "occur": "случиться",
    "occur to": "прийти в голову",
    "odd": "необычный, неправильный",
    "odds": "шансы",
    "odor (odour)": "запах",
    "offence": "обида",
    "offence (offense)": "преступление, нарушение",
    "offend": "обидеть, оскорбить",
    "offer": "предлагать, предложение",
    "officer": "служащий, должностное лицо",
    "offset": "смещение",
    "offspring": "отпрыск или всё потомство",
    "oil": "масло, нефть",
    "ominous": "зловещий",
    "omit": "пропускать",
    "on account of": "в связи",
    "on behalf of": "от имени",
    "on foot": "пешком",
    "on purpose": "нарочно",
    "on the contrary": "наоборот",
    "once": "однажды, когда-либо",
    "ongoing": "происходящий, продолжающий",
    "onion": "лук, луковица",
    "onset": "начало, наступление",
    "onward (onwards)": "далее",
    "opaque": "непрозрачный",
    "opening": "отверстие",
    "operate": "управлять, работать с",
    "opinion": "мнение",
    "opportunity": "возможность",
    "oppose": "противостоять",
    "opposite": "противоположный",
    "oppress": "угнетать, притеснять",
    "opt for": "выбирать из вариантов",
    "option": "выбор, вариант",
    "oral": "устный, ротовой",
    "orchard": "фруктовый сад",
    "order": "порядок, упорядочивать, заказ, заказывать, приказ",
    "ordinary": "обычный",
    "oriental": "восточный, азиатский",
    "origin": "происхождение, начало",
    "original": "первоначальный, первоисточник",
    "orphan": "сирота",
    "ostensibly": "по видимости, якобы",
    "otherwise": "иначе, иным образом",
    "ought": "должен, обязан",
    "ounce": "унция",
    "out loud": "вслух",
    "outcome": "результат, исход",
    "outdated": "устаревший",
    "outdoor (outdoors)": "на улице",
    "outer": "внешний",
    "outfit": "наряд, комплект, набор",
    "outgoing": "уходящий",
    "outing": "прогулка, пикник и т. д.",
    "outlaw": "преступник (особ. на свободе)",
    "outlet": "выход  (отверстие)",
    "outline": "контур, очертание, очертить",
    "outlook": "перспектива, вид,  кругозор",
    "outnumber": "превосходить численно",
    "output": "выпуск, выход",
    "outrage": "произвол, насилие, поругание",
    "outright": "прямой, откровенный",
    "outset": "начало",
    "outskirts": "окраина, окрестности",
    "outstanding": "выдающийся, неоплаченный, нерешённый",
    "outweigh": "перевешивать",
    "oven": "печь, духовка",
    "over": "заново",
    "overall": "общий, целый",
    "overcome": "преодолеть, побороть",
    "overflow": "переполняться, разлиться",
    "overhead": "над головой",
    "overhear": "подслушать, нечаянно услышать",
    "overlap": "перекрываться",
    "overlook": "не заметить",
    "overnight": "ночной, с ночёвкой",
    "override": "перекрывать, отменять",
    "overseas": "за рубежом, особ. за морем",
    "oversee": "контролировать, надзирать",
    "oversight": "надзор",
    "overt": "явный, открытый",
    "overtake": "обгонять",
    "overthrow": "свергнуть",
    "overtime": "сверхурочная работа",
    "overturn": "опрокинуть (ся)",
    "overview": "обзор",
    "overwhelm": "завалить, подавить",
    "owe": "задолжать, быть обязанным чем-л.",
    "owl": "сова",
    "own": "владеть",
    "oxygen": "кислород",
    "oyster": "устрица",
    "pace": "шаг, шагать",
    "pack": "пачка, связка, колода, и т. д., упаковывать, собирать",
    "pad": "подушечка пальца, лапы, блокнот, панель, клавиатура",
    "paddle": "грести, весло",
    "pagan": "язычник",
    "pain": "боль",
    "pair": "пара",
    "pal": "приятель",
    "palace": "дворец",
    "pale": "бледный",
    "palm": "ладонь",
    "pan": "сковорода",
    "pancake": "блин, оладья",
    "pant": "пыхтеть, задыхаться",
    "pants": "штаны",
    "paper": "бумага, документ",
    "paperwork": "оформление бумаг",
    "paragraph": "абзац, пункт",
    "parcel": "посылка",
    "pardon": "извинять, извинение",
    "parlor": "приёмная, салон",
    "parrot": "попугай",
    "parsley": "петрушка",
    "part": "роль, часть, разделять(ся)",
    "part-time": "неполная занятость",
    "participate": "участвовать",
    "particle": "частица",
    "particular": "частный, конкретный",
    "partisan": "сторонник",
    "partition": "раздел",
    "pass": "пройти",
    "pass away": "умереть",
    "pass by": "проходить мимо",
    "pass off as": "сойти, прокатить за",
    "pass out": "вырубиться",
    "pass smth on": "отдать (другому)",
    "pass up": "не воспользоваться шансом",
    "passage": "проход, коридор",
    "passenger": "пассажир",
    "passion": "страсть",
    "past": "мимо, прошлый, прошедший",
    "pastime": "времяпрепровождение",
    "pastry": "выпечка, тесто",
    "pasture": "пастбище",
    "pat": "похлопать, потрепать",
    "patch": "заплатка, накладка, латать",
    "path": "путь",
    "pathetic": "жалкий",
    "patient": "терпеливый",
    "patio": "внутренний дворик",
    "patrol": "патруль",
    "patron": "покровитель",
    "pattern": "шаблон, образец",
    "pause": "приостанавливать",
    "pave": "вымостить",
    "paw": "лапа",
    "pay back": "вернуть деньги",
    "pay off": "расплатиться (с долгом)",
    "pay out": "выплатить (крупную сумму)",
    "payment": "оплата",
    "pea": "горошина",
    "peaceful": "мирный",
    "peacekeeper": "миротворец",
    "peach": "персик",
    "peanut": "арахис",
    "pear": "груша",
    "pearl": "жемчуг, жемчужина",
    "peasant": "крестьянин",
    "pebble": "морской камень",
    "peck": "клевать",
    "peculiar": "своеобразный, особый",
    "peddle": "торговать вразнос, по мелочи",
    "pedestrian": "пешеход",
    "pee": "писать",
    "peek": "взглянуть, подсмотреть",
    "peel": "кожура, очищать",
    "peer": "ровня, ровесник, вглядываться",
    "peg": "колышек, фиксатор",
    "penalty": "наказание",
    "pending": "в ожидании, на рассмотрении",
    "penetrate": "проникать",
    "peninsula": "полуостров",
    "pepper": "перец",
    "per": "(кол-во) на (единицу)",
    "perceive": "воспринимать, ощущать",
    "perception": "восприятие",
    "perennial": "многолетний, вечный",
    "perfect": "совершенный",
    "perform": "выполнить, исполнить",
    "perhaps": "возможно",
    "peril": "опасность, угроза",
    "period": "точка (в предложении)",
    "perish": "погибать",
    "perk": "привилегия, преимущество",
    "perk up": "воспрянуть, оживиться",
    "permanent": "постоянный",
    "permit": "разрешать",
    "perpetual": "вечный, постоянный",
    "perplex": "смутить, озадачить",
    "persecute": "преследовать, травить",
    "persist": "сохраниться, упорствовать",
    "personnel": "персонал, личный состав",
    "persuade": "убеждать",
    "pervasive": "расширяющийся, всепроникающий",
    "pervert": "извращать, извращенец",
    "pest": "паразит, вредитель",
    "pet": "домашний питомец",
    "petal": "лепесток",
    "petroleum": "нефть",
    "petty": "мелкий, мелочный",
    "phony": "фальшивый",
    "physician": "врач",
    "pick": "подбирать, собирать",
    "pick on": "дразнить, докучать",
    "pick out": "выбрать или выделить из",
    "pick up": "прибрать, подобрать",
    "pickle": "мариновать, солить",
    "pie": "пирог",
    "piece": "предмет, часть, кусок",
    "pier": "пристань, пирс",
    "pierce": "проколоть",
    "pigeon": "голубь",
    "pile": "куча, стопка",
    "pile up": "сложить, навалить",
    "pill": "пилюля",
    "pillar": "столб, опора, колонна",
    "pillow": "подушка",
    "pin": "булавка, штырь, прикалывать",
    "pinch": "зажать, ущипнуть",
    "pine": "сосна",
    "pineapple": "ананас",
    "pipe": "трубка, труба",
    "piss": "писать, моча",
    "piss about": "страдать хернёй",
    "piss off": "бесить",
    "pit": "яма",
    "pitch": "подавать (мяч), ставить (напр. палатку)",
    "pity": "жалость",
    "pivot": "стержень, ось, точка опоры",
    "placement": "размещение",
    "placid": "тихий, спокойный",
    "plague": "чума",
    "plain": "ясный, очевидный, ровный, равнина, простой, незамысловатый",
    "plane": "самолёт",
    "plank": "доска, планка",
    "plant": "растение, сажать, завод, производство",
    "plaque": "бляшка, металлический знак",
    "plaster": "штукатурка или гипс",
    "plate": "плита, пластина",
    "plausible": "правдоподобный",
    "playground": "детская площадка",
    "plea": "просьба, требование, иск",
    "please": "радовать",
    "pleasure": "удовольствие, наслаждение",
    "pledge": "залог, оставлять в залог, обещание, обязательство",
    "plenty": "множество, масса, достаток",
    "plight": "тяжёлое положение, участь",
    "plot": "план, чертёж, сюжет",
    "plow": "пахать, плуг",
    "pluck": "сорвать, выдернуть",
    "plug": "штепсель, включать",
    "plug in": "включать",
    "plum": "слива",
    "plumbing": "водопровод",
    "plump": "пухлый",
    "plunge": "окунуться, нырнуть",
    "plural": "множественное число",
    "pocket": "карман, карманный",
    "point": "смысл, суть, точка, очко, острие, кончик, пункт",
    "point out": "указать",
    "poise": "равновесие, самообладание",
    "poison": "яд, травить",
    "poke": "совать, тыкать",
    "poke out": "высовывать",
    "pole": "полюс, шест, столб",
    "policy": "политика, курс",
    "polish": "шлифовать, полировать",
    "polite": "вежливый",
    "poll": "опрос или голосование",
    "pollute": "загрязнять",
    "pond": "пруд",
    "ponder": "обдумывать",
    "pool": "бассейн",
    "poor": "бедный",
    "pope": "папа римский",
    "population": "население",
    "porcelain": "фарфор",
    "porch": "крыльцо, терраса, веранда",
    "pork": "свинина",
    "portable": "портативный, переносной",
    "porter": "носильщик или порьте",
    "portray": "изображать, показывать",
    "possess": "обладать",
    "possible": "возможный",
    "post": "почта, почтовый",
    "postpone": "отложить, задержать",
    "posture": "поза",
    "pot": "горшок, кастрюля",
    "potato": "картофель",
    "potent": "мощный, могущественный",
    "pouch": "сумочка, кошелёк",
    "poultry": "домашняя птица",
    "pound": "фунт, долбить, толочь",
    "pour": "лить",
    "pour in (into)": "хлынуть, заполонить",
    "poverty": "бедность, нищета",
    "powder": "порошок",
    "powerful": "мощный, действенный",
    "practice": "практика, практиковать",
    "prairie": "степь, прерия",
    "praise": "хвалить, похвала",
    "pray": "молиться",
    "preach": "проповедовать",
    "precarious": "ненадёжный, шаткий",
    "precaution": "предосторожность",
    "precede": "предшествовать",
    "precious": "драгоценный",
    "precise": "точный",
    "preclude": "исключить, помешать",
    "predator": "хищник",
    "predecessor": "предшественник",
    "predict": "предсказывать",
    "predominant": "преобладающий",
    "prefer": "предпочитать",
    "pregnant": "беременная",
    "prejudice": "предубеждение, предрассудок",
    "preliminary": "предварительный",
    "premature": "преждевременный",
    "premise": "предпосылка",
    "premises": "помещение (-я)",
    "preoccupy": "занимать внимание",
    "prepare": "приготовить",
    "prescribe": "предписывать",
    "present": "настоящее (время), присутствующий, в наличии",
    "preserve": "сохранять, беречь",
    "preside": "председательствовать",
    "press": "нажимать",
    "presume": "предполагать",
    "pretend": "притворяться",
    "pretext": "повод",
    "pretty": "довольно-таки, приятный",
    "prevail": "преобладать, возобладать",
    "prevent": "предотвращать",
    "prevent smb from": "мешать",
    "previous": "предшествующий",
    "prey": "добыча",
    "prick": "колоть",
    "pride": "гордость",
    "priest": "жрец, священник",
    "primary": "первичный, главный",
    "prime": "важный, главный, первый",
    "primitive": "первобытный",
    "principal": "главный, основной",
    "prior": "предварительный, предшествующий",
    "prison": "тюрьма",
    "pristine": "нетронутый, первозданный",
    "privacy": "частная жизнь, конфиденциальность",
    "private": "частный, личный",
    "probable": "вероятный, возможный",
    "probe": "исследовать, проверить, датчик, зонд",
    "procedure": "процедура",
    "proceed": "следовать, продолжать",
    "process": "обработать",
    "proclaim": "провозглашать",
    "procure": "достать, добыть, обеспечить",
    "prod": "ткнуть (чтобы сдвинуть)",
    "produce": "производить",
    "proficient": "опытный, умелый",
    "profit": "польза, извлекать пользу",
    "profound": "глубокий (перен.)",
    "prohibit": "запрещать",
    "prolong": "продлить",
    "prominent": "выдающийся, видный",
    "promise": "обещать, обещание",
    "promote": "поощрять, помогать",
    "prompt": "побуждать, подстрекать, быстрый, оперативный",
    "prone": "склонный",
    "pronounce": "произносить",
    "proof": "доказательство (неисч.)",
    "prop": "подпирать, подпорка",
    "propagate": "размножать, распространять",
    "propel": "двигать, толкать",
    "proper": "правильный, надлежащий",
    "property": "собственность",
    "prophet": "пророк",
    "proponent": "сторонник, поборник",
    "propose": "предлагать",
    "prosecute": "вести, осуществлять, преследовать (законодательно)",
    "prospect": "перспектива, вид будущего",
    "prosper": "процветать, преуспевать",
    "protect": "защищать",
    "proud": "гордый, гордящийся",
    "prove": "доказывать",
    "proverb": "пословица",
    "provide": "обеспечивать, снабжать",
    "provided": "при условии",
    "provisional": "предварительный, временный",
    "proximity": "близость",
    "prudent": "разумный, осторожный",
    "pry": "любопытствовать, совать нос",
    "publish": "публиковать, издавать",
    "puddle": "лужа",
    "puff": "дуновение, пых, клубы (дыма)",
    "pull back": "отступить, отвести войска",
    "pull off": "справиться, вытянуть",
    "pull over": "притормозить ненадолго",
    "pull through": "выжить, выкарабкаться",
    "pump": "насос, нагнетать",
    "pumpkin": "тыква",
    "punch": "удар (кулаком), бить",
    "punish": "наказывать",
    "pupil": "зрачок",
    "puppet": "кукла, марионетка",
    "puppy": "щенок",
    "purchase": "покупка, покупать",
    "pure": "чистый, незагрязнённый",
    "purge": "чистка, очищать",
    "purple": "пурпурный, фиолетовый",
    "purpose": "цель, причина",
    "purse": "кошелек, сумочка",
    "pursue": "преследовать, гнаться",
    "pursuit": "преследование",
    "put aside": "отложить",
    "put away": "посадить в тюрьму",
    "put in": "вложить",
    "put off": "отвратить, отвлечь и т. д., откладывать",
    "put on": "надеть, провести (мероприятие)",
    "put out": "потушить, обременять, стеснять",
    "put smb down": "подавлять, угнетать",
    "put smth down to": "объяснить, списать на",
    "put through": "подвергнуть",
    "put together": "собирать, соединять",
    "put up": "устроить (напр. драку)",
    "put up with": "принимать, мириться",
    "puzzle": "озадачить",
    "quaint": "причудливый",
    "quality": "качество, качественный",
    "quantity": "количество",
    "quarrel": "ссора",
    "quarry": "карьер",
    "quarter": "четверть",
    "queer": "странный, чудной",
    "query": "запрос",
    "quest": "поиск",
    "question": "подвергать сомнению",
    "questionnaire": "анкета",
    "queue": "очередь",
    "quiet": "тихий, спокойный, тишина",
    "quilt": "одеяло (стёганное)",
    "quit (quit, quit)": "бросить, покинуть, перестать",
    "quite": "вполне или более-менее",
    "quiz": "викторина, тест, опрос",
    "quote": "цитировать",
    "rabbit": "кролик",
    "race": "раса, гонка",
    "rack": "стеллаж, вешалка, подставка",
    "radiate": "излучать",
    "raft": "плот",
    "rag": "тряпка",
    "rage": "ярость",
    "raid": "набег",
    "rail": "рельс, перила",
    "railway": "железная дорога",
    "rainbow": "радуга",
    "raise": "растить (детей), поднять, повысить",
    "raisin": "изюм",
    "rake": "грабли",
    "rally": "собраться, созвать (напр. армию), оживать, приходить в себя",
    "ram": "баран",
    "ramble": "бродить, блуждать",
    "ramp": "скат, съезд",
    "random": "выбранный наугад",
    "range": "диапазон, пределы",
    "rank": "ранг, разряд, звание",
    "ransom": "выкуп",
    "rape": "насиловать, изнасилование",
    "rapid": "быстрый",
    "rare": "редкий",
    "rash": "опрометчивый, необдуманный",
    "raspberry": "малина",
    "rate": "курс, тариф, ставка, показатель, уровень",
    "rather": "скорее, предпочтительнее",
    "ratio": "соотношение, пропорции",
    "rationale": "основание, обоснование",
    "rattle": "греметь, погремушка",
    "ravage": "разорить, сильно повредить",
    "raw": "сырой, необработанный",
    "ray": "луч",
    "razor": "бритва",
    "reach": "достичь",
    "reach out": "потянуться (рукой)",
    "read out": "огласить, зачитать вслух",
    "read up on": "изучить вопрос",
    "real estate": "недвижимость",
    "realize": "понимать",
    "realm": "царство или область (перен.)",
    "reap": "пожинать",
    "rear": "задний",
    "reason": "разум, благоразумие, причина, мотив",
    "rebel": "восставать",
    "rebound": "отскочить, рикошет",
    "recall": "вспоминать",
    "recede": "убывать",
    "receipt": "квитанция, получение",
    "receive": "получить",
    "recent": "недавний, последний",
    "reception": "прием, приёмная",
    "recess": "перерыв, выемка, впадина",
    "recipe": "рецепт",
    "recipient": "получатель",
    "reciprocal": "взаимный, ответный",
    "recite": "декламировать, рассказывать",
    "reckless": "опрометчивый, безрассудный",
    "reckon": "считать, рассчитывать",
    "recognize": "узнавать",
    "recollection": "воспоминание",
    "reconcile": "примириться",
    "record": "запись",
    "recount": "пересказать",
    "recover": "восстановиться, выздороветь",
    "recreation": "отдых и развлечения",
    "recruit": "набирать, вербовать",
    "rectangle": "прямоугольник",
    "recur": "повторяться, возвращаться",
    "recycle": "перерабатывать",
    "redeem": "выкупить, искупить",
    "reduce": "снижать",
    "redundant": "избыточный",
    "reed": "тростник, камыш",
    "reel": "катушка, наматывать",
    "refer": "сослаться, отослать",
    "refine": "очищать, усовершенствовать и т.д.",
    "reflect": "отражать(ся)",
    "refrain": "воздерживаться",
    "refresh": "обновлять, освежить",
    "refrigerator": "холодильник",
    "refuge": "убежище, пристанище",
    "refugee": "беженец",
    "refund": "возврат, возмещение",
    "refuse": "отказаться",
    "refute": "опровергать",
    "regain": "восстановить, вернуть себе",
    "regard": "внимание, уважение, считать, рассматривать",
    "regiment": "полк",
    "register": "касса (магазина)",
    "regret": "сожалеть, сожаление",
    "regular": "обычный",
    "rehearse": "репетировать",
    "reign": "царствовать",
    "rein": "сдерживать, держать в узде",
    "reinforce": "усилить, укрепить",
    "reject": "отвергать",
    "rejoice": "радоваться",
    "relate": "относиться, быть связанным",
    "relative": "родственник, относительный",
    "relay": "передавать, ретранслировать",
    "release": "отпускать, выпускать",
    "relentless": "настойчивый, непрекращающийся",
    "relevant": "уместный, актуальный",
    "relief": "облегчение",
    "relieve": "облегчить, освободить",
    "relish": "наслаждаться",
    "reluctant": "неохотный, нежелающий",
    "rely": "полагаться",
    "remain": "оставаться",
    "remainder": "оставшаяся часть",
    "remains": "остатки, останки",
    "remark": "замечать, замечание",
    "remedy": "средство",
    "remind": "напомнить",
    "reminiscent": "напоминающий",
    "remnant": "остаток",
    "remorse": "раскаяние",
    "remote": "отдалённый, захолустный",
    "remove": "убрать, удалить",
    "render": "делать, придавать свойство, воздавать, оказывать",
    "renew": "возобновлять",
    "renovate": "ремонтировать, обновлять",
    "renown": "известность, слава",
    "rent": "аренда, арендовать, сдавать",
    "repair": "ремонт, ремонтировать",
    "repay": "отплатить",
    "repeal": "отменить (закон)",
    "repel": "отталкивать, отражать",
    "replace": "заменить",
    "replicate": "воспроизвести",
    "reply": "ответ, отвечать",
    "report": "доклад, отчёт",
    "represent": "представлять",
    "repress": "подавлять",
    "reproduce": "воспроизводить",
    "request": "запрос, запрашивать",
    "require": "требовать",
    "rescue": "спасать, спасение",
    "research": "исследование, исследовать",
    "resemble": "походить",
    "resent": "огорчаться, возмущаться",
    "reserve": "запас, запасной, запасать, бронировать",
    "reserved": "скрытный, замкнутый",
    "reside": "проживать (где-то)",
    "residue": "остаток",
    "resign": "отказываться, сдавать, уходить",
    "resilient": "стойкий, упругий",
    "resist": "сопротивляться",
    "resolute": "решительный, твёрдый",
    "resolve": "решиться, решимость, разрешить (ситуацию)",
    "resort": "курорт",
    "resort to": "прибегать к",
    "respective": "соответственный",
    "respond": "отвечать",
    "response": "ответ, отклик",
    "responsible": "ответственный",
    "rest": "остаток, остальное, отдых, отдыхать, покоиться",
    "restore": "восстанавливать",
    "restrain": "ограничивать, удерживать",
    "restrict": "ограничивать",
    "result in": "привести к",
    "resume": "возобновить",
    "resurrect": "воскрешать",
    "retail": "розничная продажа",
    "retain": "сохранять, удерживать",
    "retaliate": "отплатить, отомстить",
    "retarded": "умственно отсталый",
    "retention": "удержание, сохранение",
    "retire": "уходить в отставку, на пенсию",
    "retract": "втягивать, брать обратно и т.д.",
    "retreat": "отступать",
    "retrieve": "извлечь, вернуть",
    "retrospect": "взгляд в прошлое",
    "return": "возвращаться",
    "reveal": "раскрыть, выявить",
    "revenge": "месть",
    "revenue": "доход, выручка",
    "revere": "почитать",
    "reverse": "обратный, обратить",
    "review": "обзор, сделать обзор",
    "revise": "пересматривать, проверять",
    "revive": "возрождать, оживлять",
    "revolt": "восстание, бунт",
    "revolve": "вращаться",
    "reward": "награда, награждать",
    "rhyme": "рифма",
    "rib": "ребро",
    "ribbon": "лента",
    "rice": "рис",
    "rich": "богатый",
    "rid": "освобождать, избавлять",
    "riddle": "загадка",
    "ride": "ездить",
    "ridge": "хребет, ребро",
    "ridicule": "осмеяние",
    "ridiculous": "смешной, нелепый",
    "rifle": "винтовка",
    "rift": "разрыв, трещина",
    "rig": "установка, сооружение",
    "right away": "немедленно",
    "righteous": "праведный",
    "rigid": "жесткий",
    "rigorous": "строгий, тщательный",
    "rim": "край, обод",
    "ring (rang, rung)": "звонить, звонок",
    "rinse": "полоскать, полоскание",
    "riot": "бунт",
    "rip": "рвать",
    "rip off": "обмануть, ободрать",
    "ripe": "созревший",
    "ripple": "рябь или пульсация",
    "rise (rose, risen)": "подниматься, повышаться",
    "rite": "церемония, ритуал",
    "rival": "соперничающий, враждебный",
    "rivet": "заклёпка, приковывать, клепать",
    "roadside": "обочина, придорожный",
    "roadway": "проезжая часть",
    "roam": "бродить, блуждать",
    "roar": "рёв, реветь",
    "roast": "жарить",
    "rob": "грабить, отнимать",
    "robe": "халат, мантия, одеяние",
    "robust": "крепкий, надёжный, здоровый",
    "rock": "камень, скала, качать, трясти",
    "rod": "стержень, прут",
    "rodent": "грызун",
    "rogue": "жулик, проходимец",
    "roll": "рулон, клубок, свиток и т. д., катиться",
    "roll up": "свернуть, скатать",
    "roof": "крыша",
    "room": "место, пространство",
    "root": "корень",
    "root out": "искоренить",
    "rope": "верёвка",
    "rope in (into)": "втянуть в занятие",
    "rot": "гнить",
    "rotate": "сменять, чередовать, вращать",
    "rotten": "гнилой",
    "rough": "грубый, неровный, жёсткий",
    "rough up": "грубо обращаться, нападать",
    "round": "круг, круглый",
    "round down": "округлить (вниз)",
    "rouse": "будить",
    "route": "маршрут",
    "row": "грести, ряд",
    "royal": "королевский",
    "rub": "тереть",
    "rubber": "резина, резиновый",
    "rubbish": "мусор (в осн. брит.)",
    "rubble": "щебень, обломки",
    "rude": "грубый, неприличный",
    "ruffle": "рябить, взъерошить",
    "rug": "коврик",
    "rugged": "неровный, грубый",
    "ruin": "разрушать, губить",
    "rule": "правило, править",
    "ruler": "линейка",
    "rumble": "грохот, рокот",
    "rumor (rumour)": "слух",
    "run": "управлять",
    "run after": "гнаться за",
    "run away": "убежать",
    "run counter": "противоречить",
    "run into": "случайно встретить",
    "run out of": "исчерпать запас",
    "run over": "переехать машиной",
    "runaway": "беглый, беглец",
    "runway": "взлётная полоса",
    "rupture": "разрыв",
    "rural": "сельский",
    "rush": "торопиться, мчаться, броситься",
    "rust": "ржавчина",
    "rustic": "деревенский, простой, грубый",
    "rustle": "шелест",
    "rut": "борозда, колея, рутина",
    "ruthless": "беспощадный",
    "sack": "мешок",
    "sacred": "священный",
    "sacrifice": "жертва",
    "saddle": "седло",
    "safe": "надёжный, защищённый",
    "sail": "парус, плыть (о судне)",
    "saint": "святой",
    "salary": "зарплата",
    "sale": "продажа, сбыт",
    "salesperson": "продавец (в магазине)",
    "salient": "выступающий, заметный",
    "saliva": "слюна",
    "salmon": "лосось",
    "salt": "соль",
    "salvage": "спасти, уберечь от потери",
    "salvation": "спасение",
    "sample": "образец, часть",
    "sanctuary": "убежище или заповедник",
    "sand": "песок",
    "sane": "здравый, вменяемый",
    "satellite": "спутник",
    "satisfy": "удовлетворять",
    "saturated": "насыщенный",
    "saucepan": "кастрюля или сковорода",
    "saucer": "блюдце",
    "sausage": "колбаса",
    "savage": "дикарь, дикий (в т. ч. перен.)",
    "save up": "накопить",
    "savings": "сбережения",
    "savior": "спаситель",
    "savor": "вкус, смаковать",
    "scaffold": "помост",
    "scale": "чешуя, шелуха, взбираться, масштаб, шкала",
    "scales": "весы",
    "scant": "скудный",
    "scar": "шрам",
    "scarce": "редкий, скудный",
    "scare": "пугать",
    "scarf": "шарф",
    "scarlet": "алый",
    "scatter": "разбрасывать, рассеивать",
    "scent": "нюх или аромат",
    "schedule": "график, расписание",
    "scheme": "схема",
    "scholar": "учёный",
    "scholarship (money)": "стипендия",
    "science": "наука",
    "scissors": "ножницы",
    "scold": "ругать",
    "scoop": "совок, ковш, черпать",
    "scope": "пределы, район",
    "score": "счёт, очко, балл",
    "scorn": "презирать, презрение",
    "scour": "обыскать, прочесать",
    "scout": "разведчик",
    "scowl": "насупиться, рассердиться",
    "scramble": "драка, схватка, карабкаться, продираться",
    "scrambled eggs": "яичница",
    "scrap": "обломок, обрывок",
    "scrape": "скрести",
    "scrape through": "с трудом пройти",
    "scrape together": "наскрести (денег)",
    "scratch": "царапать, царапина",
    "scream": "кричать, крик, вопль",
    "screech": "визг, скрип, хрип и т. д.",
    "screenplay": "сценарий",
    "screw": "винт, ввинчивать, к чёрту",
    "screw up": "завалить, просрать",
    "scribble": "криво написать, нацарапать",
    "scrub": "чистить, драить",
    "scrutiny": "внимательный осмотр, изучение",
    "scurry": "бежать, сновать, суетиться",
    "seal": "печать, запечатывать, тюлень",
    "seam": "шов",
    "search": "поиски, искать",
    "seasoning": "приправа",
    "seat": "место, сиденье",
    "secular": "светский",
    "secure": "безопасный, надёжный",
    "sediment": "осадок",
    "seduce": "соблазнять",
    "see off": "провожать (в путь)",
    "see through": "довести до конца",
    "see to": "заняться, позаботиться",
    "seed": "семя, сеять",
    "seek (sought, sought)": "искать, стремиться",
    "seem": "казаться",
    "seep": "просачиваться, проникать",
    "segregate": "разделять",
    "seize": "схватить",
    "seldom": "редко",
    "select": "выбирать, отбирать",
    "selfish": "эгоистичный",
    "sell out of": "распродать (весь товар)",
    "send (sent, sent)": "отправить",
    "send out": "рассылать",
    "senior": "старший",
    "sensation": "ощущение",
    "sense": "чувство, осознание, смысл, значение",
    "sensible": "разумный, имеющий смысл",
    "sensitive": "чувствительный",
    "sentence": "предложение, приговор",
    "sentiment": "настроение, чувство",
    "separate": "отдельный, отделять",
    "sequence": "последовательность",
    "serene": "безмятежный, безоблачный",
    "series": "сериал",
    "sermon": "проповедь",
    "serve": "служить, сидеть (в тюрьме), подавать (блюдо)",
    "set": "набор, комплект",
    "set (set, set)": "установить, наладить",
    "set about": "приступить, начать",
    "set down": "написать, установить",
    "set out": "разъяснять",
    "set out for": "отправиться",
    "set smb back": "задержать, обойтись в (сумму)",
    "set smb up": "подставить",
    "set smb up with": "устроить, организовать, познакомить, свести",
    "settle": "уладить, селиться",
    "settle for": "согласиться на",
    "settle in": "освоиться",
    "settle on": "определиться с",
    "sever": "разрывать, разъединять",
    "several": "несколько",
    "severe": "серьёзный, суровый, жёсткий",
    "sew": "шить",
    "sewage": "канализация, сточные воды",
    "sex": "пол",
    "shabby": "облезлый, убогий",
    "shack": "лачуга, хибара, будка",
    "shade": "тень, оттенок (яркости)",
    "shadow": "тень",
    "shaft": "вал, ось, стержень, шахта (напр. лифта)",
    "shake (shook, shaken)": "трясти, пожать (руки)",
    "shake off": "отделаться",
    "shake up": "встряхнуть, взбудоражить",
    "shallow": "мелкий",
    "shame": "стыд, позор",
    "shape": "форма, формировать",
    "share": "делить, доля, разделять (напр. мнение)",
    "shareholder": "акционер",
    "shark": "акула",
    "sharp": "острый",
    "shatter": "разрушить, раздробить",
    "shave": "брить",
    "shed": "сарай, ангар и т.п.",
    "shed (shed, shed)": "проливать, терять, сбрасывать",
    "sheep (pl:sheep)": "овца",
    "sheer": "явный, полный, чистый, отвесный",
    "sheet": "лист",
    "shelf (pl: shelves)": "полка, уступ",
    "shell": "скорлупа, раковина, гильза",
    "shelter": "укрытие, убежище",
    "shepherd": "пастух",
    "shield": "щит, прикрывать",
    "shift": "сменить, сдвинуть, смена (на работе)",
    "shimmer": "мерцать, переливаться",
    "shine (shone, shined)": "сиять, блестеть, светить",
    "ship": "доставлять",
    "shirt": "рубашка",
    "shiver": "дрожь, дрожать",
    "shoe": "туфля, ботинок (низкий)",
    "shoot (shot, shot)": "стрелять, снимать, фотографировать",
    "shore": "берег",
    "shortage": "нехватка",
    "shoulder": "плечо",
    "shout": "кричать",
    "shove": "резко толкнуть, пихнуть",
    "shovel": "лопата или совок",
    "show around": "показать места",
    "show off": "показать всем, похвастаться",
    "show up": "прийти, появиться",
    "showcase": "витрина, демонстрировать",
    "shower": "душ",
    "shred": "мелко порубить",
    "shrewd": "проницательный",
    "shriek": "пронзительный крик, вскрикнуть",
    "shrimp": "креветка",
    "shrine": "святыня",
    "shrink (shrank, shrunk)": "сжаться, съёжиться",
    "shroud": "пелена, окутывать",
    "shrub": "куст",
    "shrug": "пожать плечами",
    "shudder": "содрогаться, дрожь",
    "shuffle": "перемешивать, тасовать",
    "shun": "избегать",
    "shut (shut, shut)": "закрыть",
    "shut out": "не впускать",
    "shut up": "заткнуться",
    "shy": "застенчивый",
    "siblings": "братья и сестры",
    "sick": "больной, плохо чувствующий",
    "sideline": "что-то вторичное",
    "sidewalk": "тротуар",
    "sideways": "вбок",
    "siege": "осада",
    "sift": "просеять",
    "sigh": "вздох",
    "sight": "вид, поле зрения, достопримечательность",
    "sign": "знак, подпись, подписывать",
    "sign up": "записаться, зарегистрироваться",
    "signature": "подпись",
    "significant": "значительный",
    "signify": "означать",
    "silent": "тихий, молчаливый",
    "silk": "шелк",
    "sill": "подоконник, порог",
    "silly": "глупый",
    "similar": "похожий",
    "simmer": "медленно кипятить",
    "simple": "простой, обычный",
    "simulate": "моделировать, имитировать",
    "simultaneous": "одновременный",
    "sin": "грех, грешить",
    "since": "поскольку, с (какого-то времени)",
    "sincere": "искренний",
    "single": "один, отдельный, единый",
    "singular": "единственный, особый",
    "sinister": "зловещий",
    "sink": "раковина",
    "sink (sank, sunk)": "тонуть, топить",
    "sip": "глоток",
    "sit around": "просто сидеть",
    "sit back": "отсидеться",
    "sit out": "высидеть",
    "site": "местоположение",
    "situate": "располагать",
    "size up": "изучить, оценить",
    "skate": "конёк, кататься на коньках",
    "sketch": "набросок, эскиз",
    "ski": "ездить на лыжах",
    "skid": "заносить, скользить, буксовать",
    "skill": "умение, искусство и т. д.",
    "skillet": "сковорода (глубокая)",
    "skim": "скользить (легко)",
    "skin": "кожа",
    "skinny": "тощий",
    "skip": "прыгать, перепрыгивать",
    "skirt": "юбка",
    "skull": "череп",
    "slab": "плита, блок",
    "slack": "расслабление, поблажка",
    "slam": "хлопать, ударять, захлопывать",
    "slant": "наклон, скос, наклонять",
    "slap": "шлёпать, пощечина",
    "slash": "рубить, хлестать, кромсать",
    "slate": "шифер или сланец",
    "slaughter": "бойня (в т. ч. перен.), забивать",
    "slave": "раб",
    "slay (slew, slain)": "умертвить",
    "sleek": "гладкий, лоснящийся",
    "sleep around": "трахаться, блудить",
    "sleeve": "рукав",
    "slender": "тонкий, стройный, изящный",
    "slice": "ломтик, нарезать",
    "slick": "скользкий",
    "slide (slid, slid)": "скользить",
    "slight": "лёгкий, слабый, незначительный",
    "slim": "стройный",
    "slime": "слизь",
    "slip": "скользнуть",
    "slipper": "тапок",
    "slippery": "скользкий",
    "slit": "щель",
    "slope": "склон, наклон",
    "sloppy": "небрежный, неряшливый",
    "slot": "разъём, паз, гнездо",
    "slow down": "замедлиться",
    "sludge": "осадок, отстой, ил",
    "slum": "трущобы",
    "slump": "спад, сваливаться",
    "slut": "шлюха",
    "sly": "лукавый, коварный",
    "smack": "шлёпнуть, хлопнуть",
    "smart": "умный, хитрый",
    "smash": "громить, разбивать",
    "smear": "мазать",
    "smell": "запах, пахнуть, нюхать",
    "smirk": "ухмылка",
    "smooth": "гладкий",
    "smother": "задушить",
    "smudge": "смазать",
    "smuggle": "провозить тайно",
    "snack": "лёгкая закуска",
    "snag": "коряга или загвоздка",
    "snail": "улитка",
    "snake": "змея",
    "snap": "щёлкать, щелчок",
    "snapshot": "снимок",
    "snarl": "рычать, огрызаться",
    "snatch": "схватить, утащить",
    "sneak": "красться",
    "sneakers": "кеды, кроссовки",
    "sneer": "усмешка, насмехаться",
    "sneeze": "чихать",
    "sniff": "нюхать, сопеть, фы и т. д.",
    "snore": "храп, храпеть",
    "snort": "фыркнуть, хмыкнуть",
    "snowball": "снежок",
    "snug": "уютный, тесный",
    "so far": "до сих пор",
    "so-called": "так называемый",
    "soak": "мочить, пропитывать",
    "soap": "мыло",
    "soar": "парить, взмывать (втч. о ценах)",
    "sob": "рыдать, всхлипывать",
    "sober": "трезвый",
    "sober up": "протрезветь",
    "soccer": "футбол",
    "society": "общество",
    "sock": "носок",
    "socket": "разъем, розетка",
    "soften": "смягчать",
    "software": "программное обеспечение",
    "soil": "почва",
    "solar": "солнечный",
    "sole": "единственный, подошва",
    "solemn": "торжественный, серьёзный",
    "solid": "сплошной, твёрдый, крепкий",
    "solitary": "уединенный",
    "solitude": "одиночество",
    "solution": "решение, ответ, раствор",
    "solve": "решить",
    "someday": "когда-нибудь",
    "somewhat": "слегка",
    "soon": "скоро или рано",
    "soot": "сажа",
    "soothe": "успокаивать",
    "sophisticated": "сложный, изощрённый",
    "sore": "больной, рана",
    "sorrow": "горе, скорбь",
    "sound": "здоровый",
    "sour": "кислый",
    "source": "источник",
    "south": "юг, южный",
    "sovereign": "суверенный",
    "sow (sowed, sown)": "сеять",
    "space": "пространство, космос",
    "spade": "лопата",
    "span": "пролёт, интервал",
    "spare": "щадить, избавить, запасной, лишний, выделить, уделить",
    "spark": "искра, вспыхивать",
    "sparkle": "сверкать, блистать, искриться",
    "sparse": "редкий, разреженный",
    "spawn": "порождать, порождение и т. д.",
    "speak out": "высказать",
    "speak up": "говорить громче",
    "spear": "копье",
    "special": "особенный, специальный",
    "species": "вид (биол.)",
    "specific": "конкретный",
    "specify": "указать, установить, уточнить",
    "specimen": "образец, экземпляр",
    "speck": "пятнышко, крупинка",
    "spectacle": "зрелище",
    "spectator": "зритель, наблюдатель",
    "speculate": "предполагать, строить догадки",
    "speed up": "ускориться",
    "spell": "заклинание, чары, произносить (писать) по буквам",
    "spell out": "разъяснить",
    "spend (spent, spent)": "тратить, проводить (время)",
    "spice": "пряность, специя",
    "spike": "шип, штырь, острие, клин",
    "spill": "проливать, рассыпать",
    "spin": "крутить",
    "spine": "позвоночник",
    "spirit": "дух",
    "spit (spat, spat)": "плевать",
    "splash": "брызги, всплеск",
    "splendid": "великолепный, блестящий",
    "splinter": "шепка, осколок, заноза",
    "split (split, split)": "расколоться, распасться",
    "split up": "разделиться",
    "spoil": "портить",
    "spokesman": "представитель, прес-секретарь",
    "sponge": "губка",
    "spooky": "жуткий",
    "spot": "место, пятно, заметить, увидеть",
    "spotlight": "прожектор, центр внимания",
    "spouse": "супруг, супруга",
    "sprawl": "растягиваться, расползаться",
    "spread (spread, spread)": "распространять(ся)",
    "spring (sprang, sprung)": "прыгать, источник, проистекать",
    "sprinkle": "побрызгать, посыпать",
    "sprout": "росток, прорастать",
    "spur": "шпора, побуждать",
    "spy (on)": "шпион, шпионить",
    "squad": "команда, отряд",
    "square": "квадрат, квадратный, площадь",
    "squash": "раздавливать, расплющивать",
    "squat": "приседать, приземистый",
    "squeak": "пищать, скрипеть",
    "squeal": "визг, визжать",
    "squeeze": "сжимать",
    "squid": "кальмар",
    "squint": "щуриться или коситься",
    "squirm": "извиваться, ёрзать",
    "squirrel": "белка",
    "squirt": "тонкая струя",
    "stab": "ударить ножом, зарезать",
    "stable": "устойчивый",
    "stack": "стопка, куча, складывать",
    "staff": "персонал",
    "stage": "сцена, стадия, ступень",
    "stagger": "шататься, колебаться",
    "stain": "пятно, окрашиваться, пачкать",
    "stairs": "ступеньки, лестница",
    "stake": "стойка, кол, столб, ставка, делать ставку",
    "stale": "несвежий",
    "stalk": "красться, преследовать, стебель",
    "stall": "задержаться, застопориться, палатка, ларёк",
    "stamp": "печать, штамп, марка",
    "stance": "поза, положение",
    "stand": "терпеть, выносить",
    "stand against": "противостоять",
    "stand by": "поддерживать, быть наготове",
    "stand for": "выступать за, обозначать (в сокращении)",
    "stand out": "выделяться",
    "standpoint": "точка зрения",
    "staple": "скоба, скрепка",
    "stare": "пялиться, пристальный взгляд",
    "stark": "чёткий, жёсткий, острый (об. перен.)",
    "start up": "основать, установить",
    "startle": "испугать (неожиданностью)",
    "starve": "голодать",
    "stash": "тайник, заначка,  прятать",
    "state": "государство, состояние, заявить, отметить",
    "stature": "рост",
    "statutory": "установленный законом",
    "stay": "оставаться, остановиться пожить",
    "stay up": "не ложиться",
    "steady": "устойчивый",
    "steal (stole, stolen)": "красть",
    "stealth": "скрытность, хитрость",
    "steam": "пар",
    "steel": "сталь, стальной",
    "steep": "крутой",
    "steer": "управлять, направлять",
    "stellar": "звездный",
    "stem": "стебель",
    "step": "шаг, ступать, ступенька",
    "step down": "уйти в отставку",
    "step in": "вмешаться",
    "stern": "суровый, строгий",
    "stew": "тушить, рагу",
    "stick": "палка, тыкать палкой",
    "stick (stuck, stuck)": "приклеить",
    "stick at": "упорно продолжать",
    "stick out": "торчать, выделяться",
    "stick to": "придерживаться, соблюдать",
    "stick together": "держаться вместе",
    "stiff": "жёсткий, негнущийся",
    "stifle": "удушать",
    "still": "спокойный, неподвижный",
    "sting (stung, stung)": "жалить, жало",
    "stink (stank, stunk)": "вонять",
    "stir": "шевелить(ся), двигать и т.д.",
    "stir up": "вызвать, возбудить",
    "stitch": "стежок, сшить, зашить",
    "stock": "запас, акции",
    "stock up on": "запастись",
    "stocking": "чулок",
    "stomach": "желудок, живот",
    "stomp": "топать",
    "stool": "табурет",
    "stoop": "крыльцо со ступеньками (амер.), наклоняться или сутулиться",
    "stop over": "заехать, остановиться",
    "store": "хранить, хранилище, магазин",
    "stout": "толстый, плотный, крепкий",
    "stove": "плита",
    "straight": "прямой, прямо",
    "straightforward": "простой, прямой, откровенный",
    "strain": "напрягать, напряжение",
    "strait": "пролив",
    "strand": "застрять, сесть на мель, прядь, нить (об. скрученная)",
    "strangle": "придушить",
    "strap": "ремень",
    "straw": "солома",
    "strawberry": "клубника",
    "stray": "бродить, бродячий",
    "streak": "полоса, след, струя",
    "stream": "поток, ручей",
    "strength": "сила, мощность, прочность",
    "stress": "подчеркнуть, заострить внимание",
    "stretch": "тянуть, вытягивать",
    "strew (strewn, strewn)": "разбрасывать",
    "strict": "строгий, точный",
    "stride (strode, stridden)": "шагать, шаг",
    "strike": "забастовка",
    "strike (struck, struck)": "бить, удар",
    "strike out": "потерпеть неудачу",
    "string": "струна, тетива, бечева",
    "stringent": "строгий",
    "strip": "полоса, лента, снимать (обнажать)",
    "stripe": "полоса",
    "strive (strove, striven)": "стремиться, добиваться",
    "stroke": "гладить",
    "stroll": "прогулка, прогуливаться",
    "struggle": "бороться, борьба",
    "stubborn": "упрямый",
    "stud": "шпилька",
    "stuff": "вещи, всякая фигня",
    "stuffy": "душный",
    "stumble": "спотыкаться, запинаться",
    "stump": "пень",
    "stun": "ошеломить, оглушить",
    "stunt": "трюк, выходка",
    "stupid": "глупый",
    "sturdy": "крепкий",
    "subdue": "подавить, укротить",
    "subject": "предмет, тема, подверженный, подвергать",
    "sublime": "возвышенный, величественный",
    "submerge": "погружаться под воду",
    "submit": "подавать информацию",
    "submit to": "подчиниться",
    "subordinate": "подчиненный, подчинять",
    "subscribe": "подписываться",
    "subsequent": "последующий",
    "subside": "утихать, спадать",
    "subsidiary": "вспомогательный, дополнительный",
    "subsist": "существовать, прокормиться",
    "substance": "вещество, материя",
    "substantial": "существенный, значительный",
    "substantive": "существенный, основной",
    "substitute": "заменять, замена",
    "subtitle": "субтитры или подзаголовок",
    "subtle": "тонкий, неуловимый",
    "subtract": "вычитать",
    "suburb": "пригород",
    "subvert": "подорвать",
    "subway": "метро (амер.)",
    "succeed": "достичь цели",
    "success": "успех",
    "succession": "последовательность, преемственность",
    "successor": "преемник",
    "succumb": "поддаться, не устоять",
    "suck": "сосать",
    "sudden": "внезапный",
    "sue": "предъявить иск",
    "suffer": "страдать",
    "suffice": "хватать, быть достаточным",
    "sufficient": "достаточный",
    "suffocate": "задыхаться",
    "suggest": "предлагать",
    "suit": "костюм, комплект, подходить, соответствовать",
    "suitcase": "чемодан",
    "suite": "апартаменты, номер люкс",
    "sulfur (sulphur)": "сера",
    "sum": "сумма, суммировать",
    "sum up": "подытожить",
    "summary": "сводка, конспект",
    "summit": "вершина, верх",
    "summon": "вызывать",
    "sunset": "закат солнца",
    "superb": "превосходный",
    "superficial": "поверхностный",
    "superintendent": "управляющий, начальник",
    "superior": "высший, превосходящий",
    "supervise": "надзирать, контролировать",
    "supplement": "дополнение, дополнять",
    "supply": "поставлять, поставка, предложение",
    "support": "поддерживать",
    "suppose": "полагать",
    "suppress": "подавлять, удерживать",
    "supreme": "высший,  верховный",
    "sure": "уверенный",
    "surf": "прибой",
    "surface": "поверхность",
    "surge": "волна, всплеск, вздыматься",
    "surgery": "хирургия, операция",
    "surpass": "превзойти",
    "surplus": "избыток, излишек (экон.)",
    "surprise": "удивлять, удивление",
    "surrender": "сдаваться, капитулировать",
    "surround": "окружать",
    "surveillance": "наблюдение",
    "survey": "обзор, исследование",
    "survive": "выжить",
    "susceptible": "восприимчивый, подверженный",
    "suspect": "подозревать, подозреваемый",
    "suspend": "приостановить, подвесить",
    "suspicious": "подозрительный",
    "sustain": "поддерживать",
    "swallow": "проглатывать, глоток",
    "swamp": "болото, топь",
    "swan": "лебедь",
    "swap": "менять, обмен",
    "swarm": "рой, роиться, толпиться",
    "sway": "раскачивать",
    "swear": "клясться, ругаться",
    "sweat": "пот, потеть",
    "sweep": "размах, охват",
    "sweep (swept, swept)": "подметать",
    "sweetheart": "возлюбленный (ая), милый",
    "swell (swelled, swollen)": "раздуваться, распухать",
    "swift": "быстрый, скорый",
    "swing (swung, swung)": "качать, махать",
    "swirl": "кружить, вихрь, водоворот",
    "switch": "переключать, переключатель",
    "swoop": "кинуться, напасть",
    "sword": "меч",
    "syllable": "слог",
    "symbol": "символ",
    "sympathy": "сочувствие",
    "syringe": "шприц",
    "table": "таблица",
    "tablet": "плитка, пластина",
    "tack": "канцелярская кнопка, гвоздик, курс, линия поведения",
    "tackle": "принадлежности, снаряжение",
    "tag": "этикетка, ярлык",
    "tail": "хвост",
    "tailor": "портной, кроить",
    "taint": "загрязнить, испортить",
    "take": "воспринимать",
    "take a look": "взглянуть",
    "take a walk": "прогуляться",
    "take aback": "ошеломить",
    "take advantage of": "воспользоваться",
    "take after": "быть в (родителей)",
    "take apart": "разбирать",
    "take away": "отнять",
    "take care": "позаботиться, заняться",
    "take down": "записать",
    "take in": "понять, вникнуть",
    "take off": "снять, взлететь",
    "take on": "соревноваться, столкнуться, набрать (напр. рабочих)",
    "take over": "принять обязанности",
    "take part": "принимать участие",
    "take place": "происходить, бывать",
    "take up": "заняться чем-либо",
    "talk into": "уговорить",
    "talk over": "обговорить",
    "talk smb out": "отговорить",
    "tall": "высокий (чел)",
    "tally": "подсчитывать, подсчёт, итог",
    "tame": "ручной, приручать",
    "tan": "загар, загорать",
    "tangible": "осязаемый",
    "tangle": "запутывать",
    "tank": "бак",
    "tap": "стучать, кран",
    "tape": "лента, записывать на ленту",
    "tar": "смола",
    "target": "цель, мишень",
    "tart": "пирог, обычно фруктовый",
    "task": "задание, задача",
    "taste": "вкус, иметь вкус",
    "tattered": "оборванный",
    "taunt": "насмехаться, насмешка",
    "tax": "налог",
    "team": "команда, бригада",
    "tear": "слеза",
    "tear (tore, torn)": "рвать",
    "tease": "дразнить",
    "tedious": "утомительный, нудный",
    "teenage": "возраста 1-19",
    "tell apart": "различать",
    "tell from": "отличить",
    "tell off": "ругать",
    "tell on": "донести на кого-то",
    "temper": "характер",
    "temple": "храм, висок",
    "temporary": "временный",
    "tempt": "искушать",
    "tenant": "арендатор",
    "tend": "иметь склонность, ухаживать",
    "tender": "нежный",
    "tense": "напряжённый, напрягать",
    "tent": "палатка, шатёр",
    "tentative": "пробный, экспериментальный",
    "term": "срок, период, термин",
    "terminal": "конечный, заключительный",
    "terminate": "прекратить, завершить (офиц.)",
    "terms": "отношения, условия, соглашение",
    "terrain": "местность",
    "terrible": "ужасный",
    "terrify": "устрашать",
    "terror": "ужас",
    "testament": "завет, завещание",
    "testify": "свидетельствовать",
    "testimony": "показание, свидетельство",
    "text": "писать (сообщение)",
    "thank": "благодарить",
    "thaw": "оттепель, оттаивать",
    "theater (theatre)": "театр",
    "theft": "кража",
    "theme": "тема",
    "thereafter": "впоследствии",
    "thereby": "тем самым",
    "therefore": "следовательно",
    "thermal": "тепловой",
    "thick": "толстый, густой",
    "thief (pl: thieves)": "вор",
    "thigh": "бедро",
    "thin": "тонкий, худой",
    "think over": "обдумать",
    "thirst": "жажда",
    "thorn": "шип, колючка",
    "thorough": "основательный, полный",
    "though (even though)": "хотя",
    "thought": "мысль",
    "thread": "нить",
    "threat": "угроза",
    "threshold": "порог",
    "thrift": "бережливость",
    "thrill": "трепет, дрожь, дрожать",
    "thrive": "процветать, преуспевать",
    "throat": "горло",
    "throng": "толпа, толчея, толпиться",
    "throw (threw, thrown)": "бросать",
    "throw away": "выбрасывать",
    "throw up": "блевануть",
    "thrust": "сунуть, пихнуть (быстро)",
    "thud": "глухой удар",
    "thug": "бандит, головорез",
    "thumb": "большой палец",
    "thump": "колотить",
    "thunder": "гром",
    "thus": "таким образом, поэтому",
    "thwart": "помешать, сорвать",
    "tick": "галочка, поставить галочку, клещ",
    "ticket": "билет, талон",
    "tickle": "щекотать",
    "tide": "волна, поток, прилив",
    "tidy": "опрятный, аккуратный",
    "tie": "галстук, завязка и т.п., связывать, завязывать",
    "tied up": "занятый",
    "tier": "ярус",
    "tight": "тугой, тесный",
    "tile": "плитка, черепица",
    "tilt": "наклонять, наклон",
    "timber": "древесина, лесоматериалы",
    "timely": "своевременный",
    "timetable": "расписание",
    "timid": "робкий, пугливый",
    "tin": "олово или жесть",
    "tinker with": "возиться, пытаться сделать",
    "tint": "оттенок",
    "tiny": "крошечный",
    "tip": "кончик, верхушка, совет, рекомендация",
    "tip (tips)": "чаевые",
    "tip smb off": "предупредить, секретно сообщить",
    "tire (tyre)": "шина",
    "tired": "усталый",
    "tissue": "ткань, бумажная салфетка",
    "title": "заголовок",
    "toddler": "малыш (1- года)",
    "toe": "палец (ноги)",
    "toil": "тяжело трудиться",
    "token": "знак, признак, маркер",
    "tolerate": "терпеть",
    "toll": "плата, пошлина, тариф",
    "tomb": "надгробие, склеп",
    "tongue": "язык",
    "tool": "инструмент",
    "tooth (pl: teeth)": "зуб",
    "topic": "тема, предмет обсуждения",
    "topple": "опрокинуть(ся)",
    "tops": "самое большое",
    "torch": "факел, горелка",
    "torment": "мучение",
    "torture": "пытка, мучить",
    "toss": "бросать",
    "total": "полный, всеобщий",
    "touch": "связь, контакт, касаться, прикосновение",
    "tough": "жёсткий, крепкий, тяжёлый",
    "tournament": "турнир",
    "tout": "рекламировать, навязывать",
    "tow": "буксир или эвакуатор, буксировать",
    "toward (towards)": "по направлению к",
    "towel": "полотенце",
    "tower": "башня, возвышаться",
    "toy with": "подумывать о ч-л",
    "trace": "след, отслеживать",
    "track": "след, выслеживать",
    "track down": "разыскать, отследить",
    "trade": "торговля, торговый",
    "traffic light": "светофор",
    "trail": "след, тропа",
    "train": "поезд, тренировать",
    "trait": "черта, характеристика",
    "traitor": "изменник",
    "tram": "трамвай",
    "tramp": "бродяга",
    "tranquil": "спокойный",
    "transcend": "преодолеть (пределы)",
    "transcript": "расшифровка, текст",
    "transfer": "перевести, перевод",
    "transition": "переход",
    "transmit": "передавать, переносить",
    "transparent": "прозрачный",
    "trap": "ловушка, ловить",
    "trash": "мусор",
    "tray": "лоток, поднос, поддон",
    "treachery": "предательство, вероломство",
    "tread": "ступать, шагать",
    "treasure": "сокровище",
    "treat": "обращаться, иметь дело",
    "treaty": "договор (между гос-вами)",
    "trek": "поход, переход",
    "tremble": "дрожать",
    "tremendous": "громадный, страшный",
    "trench": "траншея",
    "trial": "испытание, пробный, судебный процесс",
    "triangle": "треугольник",
    "tribe": "племя",
    "tribute": "дань",
    "trick": "трюк, фокус, уловка",
    "trickle": "струйка, капать, сочиться",
    "trigger": "спусковой крючок",
    "trim": "подстригать, обрезать, ровнять",
    "trip": "поездка",
    "triple": "тройной",
    "troop": "отряд",
    "trophy": "трофей, приз",
    "troublesome": "хлопотный, беспокойный",
    "trousers": "брюки",
    "trout": "форель",
    "truck": "грузовик",
    "trump": "козырь",
    "trumpet": "труба",
    "trunk": "багажник, ствол дерева, чемодан, сундук",
    "trust": "доверять",
    "try on": "примерить",
    "try out": "испытать",
    "tub": "ванна, бадья",
    "tube": "тюбик, труба, метро (в Лондоне)",
    "tuck": "подоткнуть, заправить и т. д.",
    "tug": "тянуть, рывок",
    "tuition": "обучение, плата за обучение",
    "tumble": "перевернуться, кувыркнуться",
    "tumor (tumour)": "опухоль",
    "tuna": "тунец",
    "tune": "мелодия, мотив, настраивать",
    "turkey": "индейка, индюк",
    "turmoil": "смятение, беспорядок",
    "turn": "очередь, смена",
    "turn away": "не пустить",
    "turn down": "отказаться от предложения",
    "turn into": "превращать (ся)",
    "turn off": "выключить, свернуть (с дороги)",
    "turn on": "включить, заводить, возбуждать",
    "turn out": "оказаться",
    "turn over": "перевернуть",
    "turn up": "сделать погромче",
    "turnover": "оборот (экон.)",
    "turtle": "черепаха",
    "tutor": "репетитор, преподаватель",
    "tweak": "доделать, настроить (разг.)",
    "twig": "прут, веточка",
    "twilight": "сумерки",
    "twin": "близнец",
    "twinkle": "блеск, мерцание",
    "twirl": "вертеть",
    "twist": "скручивать",
    "twitch": "дёргаться",
    "type": "печатать, набирать текст",
    "ugly": "безобразный, уродливый",
    "ultimate": "окончательный, конечный",
    "umbrella": "зонтик",
    "unanimous": "единогласный",
    "unbelievable": "невероятный",
    "uncle": "дядя",
    "undergo": "пройти через, подвергнуться",
    "underground": "подземный",
    "underlie": "лежать в основе",
    "underline": "подчеркнуть",
    "undermine": "подрывать",
    "underneath": "под",
    "underpants": "трусы",
    "underscore": "нижнее подчеркивание",
    "undertake": "предпринимать",
    "underwear": "нижнее белье",
    "undo": "расстёгнуть, развязать, открыть",
    "union": "союз",
    "unique": "уникальный",
    "unit": "единица, элемент",
    "unite": "объединять",
    "universe": "вселенная",
    "unless": "если не",
    "up-to-date": "современный, актуальный",
    "upcoming": "предстоящий, наступающий",
    "update": "обновлять, обновление",
    "uphill": "в гору",
    "uphold": "соблюдать, поддерживать",
    "upper": "верхний",
    "upright": "вертикальный, стоячий, честный",
    "uprising (rising)": "восстание",
    "upset": "расстраивать, расстроенный",
    "upside down": "вверх ногами",
    "upwards (upward)": "вверх",
    "urban": "городской",
    "urge": "побуждать, гнать и т.д.",
    "urgent": "срочный",
    "usage": "применение",
    "used to": "когда-то бывало",
    "useful": "полезный",
    "utility": "полезность",
    "utilize": "использовать",
    "utter": "полный, абсолютный, издать, произнести",
    "vacation": "отпуск, каникулы",
    "vague": "смутный",
    "vain": "тщеславный, самолюбивый, напрасный, безрезультатный",
    "valid": "действительный",
    "valley": "долина",
    "value": "ценность, оценивать",
    "valve": "задвижка, вентиль",
    "van": "фургон",
    "vanish": "исчезнуть",
    "vapor": "пар",
    "variety": "разнообразие, множество",
    "various": "разнообразный",
    "varnish": "лак, лакировать",
    "vary": "меняться, разниться",
    "vase": "ваза",
    "vast": "просторный, простор",
    "vault": "свод, небосвод, хранилище, об. в подвале",
    "veer": "резко свернуть",
    "vegetable": "овощ",
    "vegetation": "растительность",
    "vehicle": "транспортное средство",
    "veil": "вуаль, закрывать",
    "vein": "вена",
    "velocity": "быстрота перемещения",
    "velvet": "бархат",
    "vendor": "продавец, торговец",
    "vengeance": "отмщение",
    "vent": "выход, отдушина",
    "venture": "авантюра, предприятие, решиться",
    "verb": "глагол",
    "verge": "обочина, грань, предел",
    "verify": "проверить",
    "versatile": "универсальный, разносторонний",
    "verse": "стих",
    "vessel": "сосуд, судно",
    "vest": "жилет",
    "via": "через",
    "viable": "жизнеспособный",
    "vice": "порок, недостаток",
    "vice versa": "(и) наоборот",
    "vicinity": "окрестность, близость",
    "vicious": "злобный, жестокий",
    "victim": "жертва",
    "view": "вид, взгляд",
    "vigil": "дежурство, бдение",
    "vigor": "сила, энергия, здоровье",
    "village": "деревня",
    "villain": "негодяй",
    "vine": "виноград (растение)",
    "vinegar": "уксус",
    "violate": "нарушать",
    "violent": "яростный, жестокий",
    "violet": "фиолетовый",
    "violin": "скрипка",
    "virgin": "девственница, дева",
    "virtual": "действительный, фактический",
    "virtue": "достоинство, добродетель",
    "visible": "видимый",
    "vision": "зрение или видение",
    "visit": "посетить, посещение",
    "vital": "жизненный, жизненно важный",
    "vivid": "яркий",
    "vocabulary": "словарный запас",
    "vocation": "призвание или профессия",
    "void": "недействительный, пустой",
    "volatile": "изменчивый, нестабильный",
    "volume": "громкость, том, книга, объём",
    "voluntary": "добровольный",
    "vomit": "рвота, блевать",
    "vote": "голосовать",
    "vow": "клятва, торжественное обещание",
    "voyage": "путешествие, рейс",
    "vulnerable": "уязвимый, чувствительный",
    "wad": "комок или пачка (денег)",
    "wade": "брод, переходить вброд",
    "waffle": "вафля, вафельный",
    "wag": "махать, вилять",
    "wage": "заработок, оплата",
    "wail": "вопить, вопль",
    "waist": "талия",
    "waiter": "официант",
    "waive": "отказаться, воздержаться от",
    "wake (up)": "просыпаться или будить",
    "wallet": "бумажник",
    "wallpaper": "обои",
    "walnut": "грецкий орех",
    "wander": "бродить, блуждать",
    "wane": "убывать",
    "ward": "палата, отделение, камера, округ, район",
    "ward off": "предотвратить, отогнать",
    "warden": "смотритель, надзиратель",
    "wardrobe": "шкаф, гардероб",
    "warehouse": "склад",
    "warfare": "военные действия",
    "warmth": "тепло",
    "warn": "предостерегать",
    "warp": "деформироваться",
    "warrant": "гарантировать, ручаться",
    "warrior": "воин",
    "wary": "осторожный, настороженный",
    "wash down": "запивать",
    "wash up": "умыться, помыть посуду",
    "waste": "отходы, обрезки, потери и т.п., тратить (впустую)",
    "watch": "часы (наручные)",
    "watchful": "бдительный",
    "water down": "разбавить, ослабить",
    "watercolor": "акварель",
    "waterfall": "водопад",
    "wave": "волна, махать, колебаться",
    "waver": "дрогнуть",
    "wax": "воск",
    "way": "способ",
    "weak": "слабый",
    "wealth": "богатство",
    "weapon": "оружие",
    "wear": "надевать, носить",
    "wear off": "сойти на нет, выветриться",
    "wear out": "измотать, истрепать",
    "weary": "утомленный",
    "weave": "плести, ткать",
    "wedding": "свадьба",
    "wedge": "клин, клиновидный",
    "weed": "сорняк, пропалывать",
    "weep": "рыдать",
    "weigh": "весить",
    "weight": "вес",
    "weird": "нереальный, странный",
    "welcome": "желанный, приятный",
    "weld": "сварка, сваривать",
    "welfare": "пособие, мат. помощь, благополучие",
    "well": "колодец",
    "well-being": "благополучие",
    "well-off": "обеспеченный",
    "west": "запад",
    "wet": "мокрый, мочить",
    "whack": "удар, сильно бить",
    "whale": "кит",
    "whatsoever": "совсем (не)",
    "wheat": "пшеница",
    "wheel": "колесо",
    "whereabouts": "местонахождение",
    "whereas": "тогда как",
    "whereby": "посредством чего",
    "whether": "ли",
    "whim": "каприз, прихоть",
    "whine": "ныть, скулить",
    "whip": "кнут, хлестать",
    "whirl": "вихрь, кружиться",
    "whisk": "сделать быстрое движение",
    "whisper": "шептать",
    "whistle": "свист, свистеть, свисток",
    "whiz": "ловкач, умелец (в области), просвистеть, пронестись",
    "whole": "весь, целый",
    "wholesale": "опт",
    "whore": "шлюха",
    "wicked": "злобный, безнравственный",
    "wide": "широкий, просторный",
    "widespread": "распространенный",
    "widow": "вдова",
    "width": "ширина",
    "wield": "владеть, управляться",
    "wiggle": "шевелить",
    "wilderness": "глушь, дикая местность",
    "wildlife": "дикая природа",
    "will": "воля, желание",
    "willow": "ива",
    "wince": "поморщиться, поёжиться",
    "wind (wound, wound)": "наматывать, виться, петлять",
    "wind up": "заканчивать",
    "windowpane": "оконное стекло",
    "wine": "вино",
    "wink": "подмигивать",
    "wipe": "вытирать, салфетка",
    "wipe out": "уничтожить",
    "wire": "проволока, провод",
    "wise": "мудрый",
    "wit": "ум, остроумие",
    "witch": "ведьма",
    "withdraw": "отводить, убирать",
    "wither": "увянуть, засохнуть",
    "withhold": "утаивать, удерживать",
    "within": "в течение, в пределах, внутри",
    "withstand": "выдерживать",
    "witness": "свидетель",
    "wizard": "волшебник",
    "wobble": "качаться, шататься",
    "woe": "горе",
    "wonder": "удивление, удивляться, хотеть знать",
    "woo": "ухаживать, добиваться",
    "wood": "древесина, лес",
    "wool": "шерсть, шерстяной",
    "work out": "тренироваться, получиться, сработать",
    "workshop": "мастерская, студия",
    "worldwide": "всемирный",
    "worm": "червь",
    "worry": "волновать (ся)",
    "worsen": "ухудшать",
    "worship": "почитать, поклоняться",
    "worth": "стоимость, стоящий",
    "worthwhile": "стоящий",
    "wound": "рана, ранить",
    "wrap": "заворачивать",
    "wrath": "ярость",
    "wreath": "венок",
    "wreck": "разрушать, крушение, развалина",
    "wrench": "гаечный ключ",
    "wrestle": "бороться",
    "wretched": "жалкий, несчастный",
    "wrinkle": "морщина, сморщиваться",
    "wrist": "запястье",
    "write out": "выписать (документ)",
    "wrong": "не тот",
    "wrongdoing": "проступок, правонарушение",
    "yacht": "яхта",
    "yank": "дергать",
    "yard": "двор",
    "yawn": "зевать",
    "yearn": "тосковать, жаждать",
    "yell": "кричать, вопить",
    "yet": "пока ещё (отриц.), всё же",
    "yield": "уступать, приносить (доход)",
    "youngster": "юноша, ребёнок",
    "youth": "молодёжь, молодость",
    "zeal": "рвение, усердие",
    "zipper": "молния (застёжка)",
    "zoo": "зоопарк"
}
